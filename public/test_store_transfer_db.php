...
<body>
    ...
    try {
        ...
        // 스키마 실행
        $schema_file = __DIR__ . '/../sql/store_transfer_schema.sql';
        if (file_exists($schema_file)) {
            $schema_sql = file_get_contents($schema_file);
            
            echo "<h2>스키마 실행</h2>";
            echo "<p class='info'>스키마 파일 경로: " . $schema_file . "</p>";
            echo "<p class='info'>파일 크기: " . strlen($schema_sql) . " 바이트</p>";
            ...
            foreach ($statements as $i => $statement) {
                if (!empty($statement)) {
                    echo "<h3>SQL 구문 " . ($i+1) . ":</h3>";
                    echo "<pre>" . htmlspecialchars(substr($statement, 0, 200)) . (strlen($statement) > 200 ? '...' : '') . "</pre>";
                    
                    try {
                        $result = $pdo->exec($statement);
                        echo "<p class='success'>✓ SQL 구문 " . ($i+1) . " 실행 완료 (영향받은 행: " . $result . ")</p>";
                        ...
                        if (strpos($e->getMessage(), 'already exists') !== false) {
                            ...
                            echo "<p class='error'>SQL Code: " . $e->getCode() . "</p>";
                        }
                    }
                }
            }
