<?php
// Design Ref: purchase-from-logistics — match_logistics_purchase.php에서 확정한 품목들을
// purchases/purchase_items 로 실제 저장. VAT 계산 로직은 admin/add_purchase.php와 동일하게 맞춘다.
...
if (!$lc_order_id || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $purchase_date) || empty($items)) {
...
$stmt = $conn->prepare(
    "SELECT id, store_id, status, converted_purchase_id FROM lc_orders WHERE id = ?"
);
...
// "HOME K MART LOGISTICS CENTER" 거래처 조회, 없으면 신규 생성
$supplier_name = 'HOME K MART LOGISTICS CENTER';
...
try {
    ...
    foreach ($items as $item) {
        ...
        $purchase_type = in_array($item['purchase_type'] ?? '', ['box', 'piece'], true) ? $item['purchase_type'] : 'box';
        $valid_items[] = [
    ...
    if (empty($valid_items)) {
        throw new Exception('등록할 품목이 없습니다. 모든 품목의 상품/수량/단가를 확인해주세요.');
    }
    ...
    foreach ($valid_items as $item) {
        ...
        // 물류센터 단가는 VAT 포함 원가로 간주 (매입 등록 화면 기본값과 동일)
        $vat_included   = 1;
    ...
    $mark_stmt = $conn->prepare("UPDATE lc_orders SET converted_purchase_id = ? WHERE id = ?");
    $mark_stmt->bind_param('ii', $purchase_id, $lc_order_id);
    $mark_stmt->execute();
    $mark_stmt->close();
