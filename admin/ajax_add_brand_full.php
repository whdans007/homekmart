...
if (empty($name_en) && empty($name_ko)) {
    echo json_encode(['success' => false, 'message' => '브랜드명을 입력해주세요.']);
    exit;
}
...
try {
    ...
    if (!empty($where_conditions)) {
        $check_sql = "SELECT id, " . 
                     ...
                     " FROM brands WHERE " . implode(" OR ", $where_conditions);
        ...
        if ($existing_brand = $check_result->fetch_assoc()) {
            ...
            echo json_encode([
                ...
                'data' => [
                    ...
                    'name_ko' => isset($existing_brand['name_ko']) ? $existing_brand['name_ko'] : 
                                (isset($existing_brand['name']) ? $existing_brand['name'] : null),
                    'name_en' => isset($existing_brand['name_en']) ? $existing_brand['name_en'] : null
                ]
            ]);
            $check_stmt->close();
            $conn->close();
    ...
    if (!$has_id_auto) {
        ...
        $max_id_result = $conn->query("SELECT MAX(id) as max_id FROM brands");
        $max_id_row = $max_id_result->fetch_assoc();
        $next_id = ($max_id_row['max_id'] ?? 0) + 1;
        
        if ($has_name_ko && $has_name_en) {
            if ($has_created_at) {
                $insert_sql = "INSERT INTO brands (id, name_ko, name_en, created_at) VALUES (?, ?, ?, NOW())";
                $insert_stmt = $conn->prepare($insert_sql);
                $insert_stmt->bind_param("iss", $next_id, $name_ko, $name_en);
            } else {
                $insert_sql = "INSERT INTO brands (id, name_ko, name_en) VALUES (?, ?, ?)";
                $insert_stmt = $conn->prepare($insert_sql);
                $insert_stmt->bind_param("iss", $next_id, $name_ko, $name_en);
            }
        } else if ($has_name_ko) {
            if ($has_created_at) {
                $insert_sql = "INSERT INTO brands (id, name_ko, created_at) VALUES (?, ?, NOW())";
                $insert_stmt = $conn->prepare($insert_sql);
                $insert_stmt->bind_param("is", $next_id, $name_ko);
            } else {
                $insert_sql = "INSERT INTO brands (id, name_ko) VALUES (?, ?)";
                $insert_stmt = $conn->prepare($insert_sql);
                $insert_stmt->bind_param("is", $next_id, $name_ko);
            }
        } else if ($has_name) {
            ...
            if ($has_created_at) {
                $insert_sql = "INSERT INTO brands (id, name, created_at) VALUES (?, ?, NOW())";
                $insert_stmt = $conn->prepare($insert_sql);
                $insert_stmt->bind_param("is", $next_id, $name_value);
            } else {
                $insert_sql = "INSERT INTO brands (id, name) VALUES (?, ?)";
                $insert_stmt = $conn->prepare($insert_sql);
                $insert_stmt->bind_param("is", $next_id, $name_value);
            }
        } else {
    ...
    if (!$insert_stmt->execute()) {
        throw new Exception("브랜드 추가 실패: " . $insert_stmt->error);
    }
    ...
    echo json_encode(['success' => false, 'message' => '브랜드 추가 중 오류가 발생했습니다: ' . $e->getMessage()]);
}
?>