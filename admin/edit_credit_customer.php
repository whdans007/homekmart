...
$page_title = '외상거래처 수정 - ' . t('company.name');
...
try {
    ...
    // 현재 점포의 거래처만 조회
    $stmt = $pdo->prepare("SELECT * FROM credit_customers WHERE id = ? AND store_id = ? AND is_active = 1");
    ...
    if (!$customer) {
        $_SESSION['flash'] = [
            'type' => 'error',
            'message' => '외상거래처를 찾을 수 없습니다.'
        ];
        header('Location: credit_customer_management.php');
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    if ($delete_customer_id > 0 && $delete_customer_id == $customer_id) {
        try {
            ...
            error_log("Credit customer delete error: " . $e->getMessage());
        }

        // 오류가 있었다면 현재 페이지로 리다이렉트
        header("Location: edit_credit_customer.php?id=$delete_customer_id");
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            $check_stmt = $pdo->prepare("SELECT COUNT(*) FROM credit_customers WHERE name = ? AND id != ? AND store_id = ? AND is_active = 1");
            ...
            if ($check_stmt->fetchColumn() > 0) {
                ...
                if ($stmt->execute([$name, $phone, $address, $memo, $customer_id])) {
                    $_SESSION['flash'] = [
                        'type' => 'success',
                        'message' => '외상거래처 정보가 성공적으로 수정되었습니다.'
                    ];
                    header('Location: credit_customer_management.php');
                    exit;
                } else {
                    $errors[] = '외상거래처 수정 중 오류가 발생했습니다.';
                }
            }
        } catch (PDOException $e) {
    ...
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2">
                    <li>
                        <a href="credit_customer_management.php" class="text-gray-400 hover:text-gray-600">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="text-gray-600">외상거래처 수정</span>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <div class="bg-white shadow-sm rounded-lg border">
            <div class="px-6 py-4 border-b border-gray-200">
                ...
                <p class="mt-1 text-sm text-gray-600">외상거래처 정보를 수정해주세요.</p>
            </div>
                ...
                <form method="POST" class="space-y-6">
                    ...
                    <div class="flex justify-between pt-4">
                        ...
                        <div class="flex space-x-4">
                            <a href="credit_customer_management.php" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div class="p-6">
                ...
                <div class="text-center">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 mb-2">외상거래처 삭제</h3>
                    <div class="text-sm text-gray-500 mb-4">
                        <p><strong><?php echo htmlspecialchars($customer['name']); ?></strong> 외상거래처를 삭제하시겠습니까?</p>
                        <p class="font-semibold text-red-600 mt-2">삭제된 외상거래처는 복구할 수 없습니다.</p>
                    </div>
                </div>
