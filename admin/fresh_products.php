...
$stmt = $conn->prepare(
    "SELECT fp.id, fp.code, fp.name_ko, fp.name_en, fp.fresh_category, fp.sale_type, fp.unit_step_g,
            fp.pkg_weight_kg, fp.pkg_pieces_per_box, fp.unit_type, fp.price_per_100g, fp.box_sale_price,
            fp.status,
...
function fresh_compact_number($value, int $decimals = 2): string
{
...
<div class="w-full px-2 sm:px-3 md:px-4 py-8">
    ...
    <section class="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        <?php foreach ($products as $product):
            ...
            $unitCostSuffix = $product['sale_type'] === 'piece'
                ? (in_array($product['unit_type'] ?? '', ['pcs', 'pack'], true) ? $product['unit_type'] : 'pcs')
                : 'kg';
            $boxWeightDisplay = $product['pkg_weight_kg'] !== null
                ? fresh_compact_number($product['pkg_weight_kg'])
                : null;
        ?><tr class="border-t border-gray-100 cursor-pointer transition-colors hover:bg-blue-50 focus:bg-blue-50 focus:outline-none" role="link" tabindex="0" onclick="window.location.href='edit_fresh_product.php?id=<?php echo (int)$product['id']; ?>'" onkeydown="if(event.key==='Enter'){window.location.href='edit_fresh_product.php?id=<?php echo (int)$product['id']; ?>'}"><td class="px-4 py-3 font-mono text-xs"><?php echo fresh_h($product['code']); ?></td><td class="px-4 py-3"><div class="font-semibold"><?php echo fresh_h($product['name_ko']); ?></div><div class="text-xs text-gray-500"><?php echo fresh_h($product['name_en']); ?></div></td><td class="px-4 py-3"><?php echo $product['sale_type'] === 'weight' && $boxWeightDisplay !== null ? fresh_h($boxWeightDisplay) . 'kg' : ($product['sale_type'] === 'piece' && $product['pkg_pieces_per_box'] !== null ? fresh_compact_number($product['pkg_pieces_per_box']) . ' / box' : '-'); ?></td><td class="px-4 py-3"><?php echo fresh_h(fresh_category_label($product['fresh_category'])); ?></td><td class="px-4 py-3 text-center"><span class="px-2 py-1 rounded-full text-xs <?php echo $product['sale_type'] === 'piece' ? 'bg-purple-100 text-purple-700' : 'bg-sky-100 text-sky-700'; ?>"><?php echo $product['sale_type'] === 'piece' ? fresh_h(t('mall_fresh_products.sale_type_piece')) : fresh_h(t('mall_fresh_products.sale_type_weight')); ?></span></td><td class="px-4 py-3 text-right text-gray-500"><?php echo $product['latest_box_cost'] !== null ? fresh_compact_number($product['latest_box_cost']) : '-'; ?></td><td class="px-4 py-3 text-right text-gray-500"><?php echo $unitCostValue !== null ? fresh_compact_number($unitCostValue) . ($unitCostSuffix !== '' ? ' / ' . $unitCostSuffix : '') : '-'; ?></td><td class="px-4 py-3 text-right text-gray-700"><?php echo $product['box_sale_price'] !== null ? fresh_compact_number($product['box_sale_price']) : '-'; ?></td><td class="px-4 py-3 text-right text-gray-700"><?php echo $product['price_per_100g'] !== null && (float)$product['price_per_100g'] > 0 ? fresh_compact_number($product['price_per_100g']) : '-'; ?></td><td class="px-4 py-3 text-center"><span class="px-2 py-1 rounded-full text-xs <?php echo $product['status'] === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600'; ?>"><?php echo $product['status'] === 'active' ? fresh_h(t('common.active')) : fresh_h(t('common.inactive')); ?></span></td><td class="px-4 py-3"><div class="flex justify-end gap-3"><form method="post" onclick="event.stopPropagation();" onkeydown="event.stopPropagation();" onsubmit="return confirm('<?php echo fresh_h(t('mall_fresh_products.delete_confirm')); ?>');"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?php echo (int)$product['id']; ?>"><button class="text-red-600"><?php echo fresh_h(t('common.delete')); ?></button></form></div></td></tr><?php endforeach; ?>
        </tbody></table></div>
    </section>
</div>
