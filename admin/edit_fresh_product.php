...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = $conn->prepare('UPDATE mall_fresh_products SET code = ?, name_ko = ?, name_en = ?, fresh_category = ?, sale_type = ?, unit_step_g = ?, price_per_100g = ?, box_sale_price = ?, pkg_weight_kg = ?, pkg_pieces_per_box = ?, status = ? WHERE id = ?');
        $stmt->bind_param('sssssidddisi', $code, $nameKo, $nameEnValue, $freshCategory, $saleType, $unitStep, $price, $boxSalePrice, $pkgWeightKg, $pkgPiecesPerBox, $status, $id);
        $stmt->execute();
        $stmt->close();
        $conn->close();
...
$stmt = $conn->prepare('SELECT id, code, name_ko, name_en, fresh_category, sale_type, unit_step_g, price_per_100g, box_sale_price, pkg_weight_kg, pkg_pieces_per_box, status FROM mall_fresh_products WHERE id = ?');
