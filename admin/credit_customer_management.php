...
$page_title = t('navigation.credit_customer_management') . ' - ' . t('company.name');
...
try {
    ...
    // 전체 거래처 수 계산
    $total_stmt = $pdo->prepare("SELECT COUNT(id) FROM credit_customers" . $where_clause);
...
<div class="w-full px-2 sm:px-3 md:px-4 py-8">
        ...
        <form method="GET" class="flex items-center space-x-3">
            <div class="flex-1">
                <input type="text" name="search" id="search" value="<?php echo htmlspecialchars($search_term); ?>"
                       placeholder="<?php echo htmlspecialchars(t('credit_customer_management.search_placeholder')); ?>"
            ...
            <a href="credit_customer_management.php" class="px-3 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 text-sm">
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden ring-1 ring-gray-400">
        ...
        <div class="px-6 py-12 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-2"><?php echo htmlspecialchars(t('credit_customer_management.no_customers')); ?></h3>
            <p class="text-gray-600 mb-4"><?php echo htmlspecialchars(t('credit_customer_management.add_first')); ?></p>
            <a href="add_credit_customer.php" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">
                <i class="fas fa-plus mr-2"></i>
                <?php echo htmlspecialchars(t('credit_customer_management.add_customer')); ?>
            </a>
        </div>
        <?php else: ?>
        <!-- 테이블 헤더 -->
        <div class="px-6 py-4 border-b border-gray-200 bg-white flex justify-between items-center">
            <h3 class="text-lg leading-6 font-semibold text-gray-900">
                <?php echo t('navigation.credit_customer_management'); ?>
            </h3>
            <div class="flex space-x-3">
                <a href="add_credit_customer.php" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                    <i class="fas fa-plus mr-2"></i>
                    <?php echo htmlspecialchars(t('credit_customer_management.add_customer')); ?>
                </a>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full">
                ...
                <tbody class="bg-white">
                    ...
                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors duration-150 cursor-pointer" onclick="window.location.href='edit_credit_customer.php?id=<?php echo $customer['id']; ?>';">
                ...
                <div>
                    <p class="text-sm text-gray-700">
                        ...
                        echo htmlspecialchars(str_replace(
                            ...
                            t('credit_customer_management.pagination_showing')
                        ));
                        ?>
                    </p>
                </div>
