...
if ($store_id > 0) {
    ...
    $deleted_at_columns = $conn->query("SHOW COLUMNS FROM purchases LIKE 'deleted_at'");
    if ($deleted_at_columns) {
        $has_purchase_deleted_at = $deleted_at_columns->num_rows > 0;
        $deleted_at_columns->free();
    }
