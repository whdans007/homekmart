...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT * FROM price_events WHERE id = ? AND store_id = ? AND status = 'active'
    ");
    ...
    // 행사 상태 종료 처리
    $pdo->prepare("
        UPDATE price_events SET status = 'ended', ended_at = NOW(), ended_by = ?
        WHERE id = ?
    ")->execute([$user_id, $event_id]);

    $pdo->commit();

    echo json_encode(['success' => true, 'message' => '행사가 종료되었습니다. 기존 가격으로 복원되었습니다.']);

} catch (PDOException $e) {
    if (isset($pdo) && $pdo->inTransaction()) $pdo->rollBack();
    error_log("ajax_end_price_event error: " . $e->getMessage());
