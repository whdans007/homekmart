...
try {
    ...
    $inbound_unit   = kw_valid_unit($row['inbound_unit'], LC_UNIT_PCS);
    ...
    foreach ($fields as $field => $value) {
        switch ($field) {
            ...
            case 'quantity':
                ...
                break;
            case 'inbound_unit':
                $inbound_unit = kw_valid_unit($value, $inbound_unit);
                break;
            case 'pieces_per_box':
    ...
    $cost_price_pcs = kw_is_bundle_unit($inbound_unit)
        ? kw_pcs_cost($cost_price, $pieces_per_box)
        : round($cost_price, 4);
    ...
    // FR-13: lot 단위(kw_inventory)도 일관 갱신
    $st = $conn->prepare(
        "UPDATE kw_inventory SET lot_number=?, expiry_date=?, quantity_in=?, unit=? WHERE inbound_id=?"
    );
    ...
    $expiry_class   = kw_expiry_class($expiry_date);
    ...
    if (kw_is_bundle_unit($inbound_unit) && $cost_price_pcs > 0) {
