...
kw_require_staff(); // 비인증 시 리다이렉트 → 클라이언트는 JSON 파싱 실패로 조용히 무시 (NFR-2)
...
try {
    ...
    $st = $conn->prepare(
        "SELECT o.id,
                ...
                o.total_amount,
                o.created_at,
                (SELECT COUNT(*) FROM kw_order_items oi WHERE oi.order_id = o.id) AS item_count
         FROM kw_orders o
    ...
    $latest_id = (int)$conn->query("SELECT IFNULL(MAX(id), 0) FROM kw_orders WHERE status = 'pending'")->fetch_row()[0];
    $conn->close();
