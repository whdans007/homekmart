<?php
$page_title = "Product Master - KIM'S MALL WAREHOUSE";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'toggle_active') {
    ...
    if ($pid > 0) {
        $conn = get_lc_db();
        $st = $conn->prepare("UPDATE kw_products SET is_active = 1 - is_active WHERE id = ?");
        $st->bind_param('i', $pid);
        $st->execute();
        $conn->close();
    }
                ...
                <div class="border-b border-gray-100 pb-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="barcode_logistics" placeholder="KIM'S MALL WAREHOUSE Code"
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-teal-500">
                        </div>
                    </div>
                </div>
...
(function() {
    ...
    function saveListScroll() {
        try { sessionStorage.setItem('kw_products_scroll', scrollBody ? scrollBody.scrollTop : 0); } catch (e) {}
    }
    ...
    (function() {
        var saved = null;
        try { saved = sessionStorage.getItem('kw_products_scroll'); } catch (e) {}
        if (saved !== null && scrollBody) {
            scrollBody.scrollTop = parseInt(saved, 10) || 0;
            try { sessionStorage.removeItem('kw_products_scroll'); } catch (e) {}
        }
    })();
