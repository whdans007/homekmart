...
// ── Test 1: kw_normalize_unit (Design §8.2 #1) ──────────────────
echo "[Test 1] kw_normalize_unit\n";
check("'박스' → BOX",  kw_normalize_unit('박스') === 'BOX');
check("'BOX' → BOX",   kw_normalize_unit('BOX') === 'BOX');
check("'box ' → BOX",  kw_normalize_unit('box ') === 'BOX');
check("'EA' → PCS",    kw_normalize_unit('EA') === 'PCS');
check("null → PCS",    kw_normalize_unit(null) === 'PCS');
check("kw_valid_unit('pcs') → PCS", kw_valid_unit('pcs') === 'PCS');
check("kw_valid_unit('XXX') → PCS(기본)", kw_valid_unit('XXX') === 'PCS');
echo "\n";

// ── Test 2: kw_pcs_cost (Design §8.2 #2) ────────────────────────
echo "[Test 2] kw_pcs_cost\n";
check("100/20 = 5.0",   abs(kw_pcs_cost(100, 20) - 5.0) < 0.0001);
check("100/0 = 100 (ppb 보정)", abs(kw_pcs_cost(100, 0) - 100.0) < 0.0001);
check("100/3 = 33.3333", abs(kw_pcs_cost(100, 3) - 33.3333) < 0.0001);
echo "\n";
...
try {
    // 테스트 상품 생성 (ppb=20)
    $conn->query("INSERT INTO kw_products (name_en, unit, pieces_per_box, is_active) VALUES ('__TEST_BOXPCS__', 'BOX', 20, 1)");
    $pid = $conn->insert_id;

    // BOX 5개 입고 (BOX원가 100 → PCS원가 5)
    $today = date('Y-m-d');
    $pcs_cost = kw_pcs_cost(100.0, 20);
    $st = $conn->prepare(
    ...
    echo "[Test 3] BOX 5 입고 → kw_get_stock_by_unit\n";
    $stock = kw_get_stock_by_unit($conn, $pid);
    check("BOX=5, PCS=0", $stock['BOX'] === 5 && $stock['PCS'] === 0, json_encode($stock));
    check("kw_format_stock = '5 BOX'", kw_format_stock($stock) === '5 BOX', kw_format_stock($stock));
    echo "\n";

    // ── Test 4: 개봉 — 1박스 개봉, 파손 1 (Design §8.2 #4) ──────────
    // kw_execute_box_break: ajax/box_break.php와 동일한 실제 경로 검증
    echo "[Test 4] 1박스(ppb20) 개봉, 파손 1 → BOX 4 + PCS 19\n";
    $res = kw_execute_box_break($conn, $box_lot_id, 1, 1, 'L1 test');
    ...
    $ded = kw_unit_fifo_ship_allow_negative($conn, $pid, 10, 'PCS');
    $stock = kw_get_stock_by_unit($conn, $pid);
    ...
    $ded = kw_unit_fifo_ship_allow_negative($conn, $pid, 2, 'BOX');
    $stock = kw_get_stock_by_unit($conn, $pid);
    ...
    $res = kw_execute_box_break($conn, $box_lot_id, 99, 0);
    check("잔여 초과 개봉 거부", $res['success'] === false, json_encode($res));
    $res = kw_execute_box_break($conn, $box_lot_id, 1, 21);
    ...
    $res = kw_execute_box_break($conn, $box_lot_id, 1, 20, '전량 파손 테스트');
    ...
    $cnt = (int)$conn->query("SELECT COUNT(*) FROM kw_box_breaks WHERE product_id = $pid")->fetch_row()[0];
