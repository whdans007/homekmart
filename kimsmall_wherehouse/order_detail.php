...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'approve') {
            ...
            if (!$approved) {
                $conn->rollback();
                kw_set_flash('error', 'Only pending orders can be approved (it may already be processed).');
            } else {
                kw_allocate_order_stock($conn, $id, true);
                // 원가 확정 후 총액 재계산 (kw_order_items.total_amount STORED 컬럼 자동 반영)
                $conn->query(
                    "UPDATE kw_orders SET total_amount=(SELECT COALESCE(SUM(total_amount),0) FROM kw_order_items WHERE order_id=$id) WHERE id=$id"
                );
                $conn->commit();
                kw_set_flash('success', 'Order approved. Inventory deducted and cost finalized.');
            }

        } elseif ($action === 'ship') {
            // 대책 B: 재고는 승인 시 이미 차감됨 → 출고는 상태 전환만 수행한다.
            $st = $conn->prepare(
                "UPDATE kw_orders SET status='shipped', shipped_at=? WHERE id=? AND status='approved'"
            );
            $st->bind_param('si', $now, $id);
            $st->execute();
            if ($st->affected_rows > 0) {
                kw_set_flash('success', 'Outbound processing complete.');
            } else {
                kw_set_flash('error', 'Only approved orders can be processed for outbound.');
            }
            $st->close();

        } elseif ($action === 'edit_items') {
            // 승인 상태 주문의 단가/수량 수정 (출고 전)
            $st = $conn->prepare("SELECT status FROM kw_orders WHERE id = ?");
            $st->bind_param('i', $id); $st->execute();
            $cur = $st->get_result()->fetch_assoc(); $st->close();

            if (!$cur || !in_array($cur['status'], ['pending', 'approved'], true)) {
                kw_set_flash('error', 'Only received (pending) or approved orders can be modified.');
            } else {
                ...
                $has_items = false;
                $upd = $conn->prepare("UPDATE kw_order_items SET quantity = ?, unit_price = ?, order_unit = ? WHERE id = ? AND order_id = ?");
                $del = $conn->prepare("DELETE FROM kw_order_items WHERE id = ? AND order_id = ?");
                ...
                if (!$has_items) {
                    $conn->rollback();
                    kw_set_flash('error', 'At least 1 product must remain. Use order cancellation for full cancellation.');
                } else {
                    // 승인 상태만: 변경된 수량으로 재차감(단가는 수동 입력값 유지 → $set_cost=false)
                    if ($is_approved) { kw_allocate_order_stock($conn, $id, false); }
                    $conn->query(
                        "UPDATE kw_orders SET total_amount =
                         (SELECT COALESCE(SUM(total_amount),0) FROM kw_order_items WHERE order_id = $id)
                         WHERE id = $id"
                    );
                    $conn->commit();
                    kw_set_flash('success', 'Order quantity updated.');
                }
            }
            ...
            $stx = $conn->prepare("SELECT status FROM kw_orders WHERE id = ? AND deleted_at IS NULL");
            $stx->bind_param('i', $id); $stx->execute();
            $cur = $stx->get_result()->fetch_assoc(); $stx->close();

            if (!$cur) {
                kw_set_flash('error', 'Order not found.');
                ...
                if (!$prod) {
                    kw_set_flash('error', 'Product not found.');
                } else {
                    ...
                    try {
                        ...
                        if ($need_deduct) {
                            ...
                            $lots = kw_fifo_ship_allow_negative($conn, $product_id, $qty);
                            ...
                            $up = $conn->prepare("UPDATE kw_order_items SET unit_price = ? WHERE id = ?");
                        ...
                        $conn->commit();
                        kw_set_flash('success', 'Item added.' . ($need_deduct ? ' Inventory deducted (FEFO).' : ''));
                    } catch (Exception $e) {
                        $conn->rollback();
                        kw_set_flash('error', 'Failed to add item: ' . $e->getMessage());
                    }
                }
            }
            ...
            $st = $conn->prepare(
                "UPDATE kw_orders SET status='delivered', delivered_at=? WHERE id=? AND status='shipped'"
            );
            $st->bind_param('si', $now, $id);
            $st->execute();
            kw_set_flash('success', 'Delivery completed.');
            ...
            if (!kw_is_admin()) {
                ...
                $st = $conn->prepare(
                    "UPDATE kw_orders SET status='approved', shipped_at=NULL, delivered_at=NULL WHERE id=? AND status='delivered'"
                );
                $st->bind_param('i', $id);
                $st->execute();
                if ($st->affected_rows > 0) {
                    kw_set_flash('success', 'Delivery reverted. The order returned to Approved status (inventory remains deducted).');
                } else {
                    kw_set_flash('error', 'Only delivered orders can be reverted.');
                }
                $st->close();
            }
            ...
            if ($st->affected_rows > 0) {
                $st->close();
                kw_restore_order_stock($conn, $id);
                $conn->commit();
                kw_set_flash('success', 'Order cancelled.');
            } else {
                $st->close();
                $conn->rollback();
                kw_set_flash('error', 'This order cannot be cancelled.');
            }
            ...
            if ($st->affected_rows > 0) {
                ...
                kw_restore_order_stock($conn, $id);
                $conn->commit();
                kw_set_flash('success', 'Cancellation request approved.');
            } else {
                $st->close();
                $conn->rollback();
                kw_set_flash('error', 'No cancellation request to approve.');
            }

        } elseif ($action === 'reject_cancel') {
            $st = $conn->prepare(
                "UPDATE kw_orders SET status='approved' WHERE id=? AND status='cancel_requested'"
            );
            $st->bind_param('i', $id);
            $st->execute();
            kw_set_flash('success', 'Cancellation request rejected. Order restored to approved status.');
            ...
            if (!kw_is_admin()) {
                ...
                $st = $conn->prepare(
                    "UPDATE kw_orders SET deleted_at=?, deleted_by=? WHERE id=? AND status IN ('cancelled','delivered') AND deleted_at IS NULL"
                );
                $st->bind_param('sii', $now, $uid, $id);
                $st->execute();
                if ($st->affected_rows > 0) {
                    kw_set_flash('success', "Order #" . str_pad($id, 4, '0', STR_PAD_LEFT) . " moved to trash. You can restore it from the Deleted tab in the order list.");
                } else {
                    kw_set_flash('error', 'Only cancelled or delivered orders can be deleted.');
                }
                $st->close();
            }
            ...
            if (!kw_is_admin()) {
                ...
                $st = $conn->prepare(
                    "UPDATE kw_orders SET deleted_at=NULL, deleted_by=NULL WHERE id=? AND deleted_at IS NOT NULL"
                );
                $st->bind_param('i', $id);
                $st->execute();
                if ($st->affected_rows > 0) {
                    kw_set_flash('success', "Order #" . str_pad($id, 4, '0', STR_PAD_LEFT) . " restored.");
                } else {
...
try {
    ...
    if (kw_is_store_user() && $order['store_id'] != kw_current_store_id()) {
        $conn->close();
        kw_set_flash('error', 'Access denied.');
        header('Location: ' . LC_BASE . '/orders.php'); exit;
    }
    ...
    if ($order['deleted_at'] && !kw_is_admin()) {
        $conn->close();
        kw_set_flash('error', 'Order not found.');
        header('Location: ' . LC_BASE . '/orders.php'); exit;
    }
    ...
    if (in_array($order['status'], ['approved', 'cancel_requested', 'shipped', 'delivered'])) {
        ...
        $lot_rows = $conn->query(
            "SELECT oll.order_item_id, oll.quantity, oll.cost_price,
             ...
             JOIN kw_inventory inv ON oll.inventory_id = inv.id
    ...
    $picking_preview = [];
    if ($order['status'] === 'pending' && kw_is_staff()) {
        foreach ($items as $item) {
            $preview = kw_fefo_preview($conn, (int)$item['product_id'], (int)$item['quantity']);
...
<div class="flex items-center gap-3 mb-6 no-print">
    ...
    <span class="text-sm px-3 py-1 rounded-full font-medium <?php echo kw_status_class($order['status']); ?>">
        <?php echo kw_status_label($order['status']); ?>
    </span>
...
<div class="print-only" style="display:none;">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px;padding-bottom:12px;border-bottom:2px solid #000;">
        ...
        <div style="text-align:right;font-size:0.8rem;color:#555;">
            ...
            <div>Home K Mart — KIM'S MALL WAREHOUSE</div>
        </div>
    </div>
</div>
...
<?php
    ...
    $can_edit_items = in_array($order['status'], ['pending', 'approved'], true) && kw_is_staff();
?>
<div class="bg-white rounded-lg border border-gray-200 overflow-hidden mb-6">
    <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2 no-print">
            <?php if (kw_is_staff() && empty($order['deleted_at'])): ?>
            ...
            <?php endif; ?>
            <?php if ($order['status'] === 'pending' && kw_is_staff()): ?>
            ...
            <?php endif; ?>
            <?php if ($order['status'] === 'approved' && kw_is_staff()): ?>
            <form method="post" class="inline-block">
                <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(kw_csrf_token()); ?>">
                ...
                <td class="px-4 py-3 text-xs text-center">
                    ...
                    <div class="space-y-1">
                        ...
                        <?php
                            ...
                            $cls    = $expiry ? kw_expiry_class($expiry) : 'text-gray-500';
                            $qty    = $row['quantity'];
                        ?>
...
<div class="flex gap-3 flex-wrap no-print">
    ...
    <?php endif; ?>
    <?php if ($order['status'] === 'delivered' && kw_is_admin()): ?>
    <form method="post" class="inline">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(kw_csrf_token()); ?>">
    ...
    <?php endif; ?>
    <?php if (in_array($order['status'], ['cancelled', 'delivered']) && empty($order['deleted_at']) && kw_is_admin()): ?>
    <form method="post" class="inline">
        <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(kw_csrf_token()); ?>">
