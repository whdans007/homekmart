 ...
 * kw_restore_order_stock()을 트랜잭션 안에서 실제로 실행해 재고가 되돌아오는지 측정한 뒤
 * 반드시 ROLLBACK 한다 → DB 데이터는 전혀 변경되지 않는다.
 *
 * 사용법: /kimsmall_wherehouse/diag_cancel_restore.php?order_id=170
...
function stock_snapshot(mysqli $conn, array $product_ids): array {
    ...
    $rows = $conn->query(
        "SELECT product_id, COALESCE(SUM(quantity_remain),0) AS remain
         FROM kw_inventory WHERE product_id IN ($csv) GROUP BY product_id"
...
try {
    ...
    // 주문 헤더
    $st = $conn->prepare("SELECT id, status, store_id, shipped_at, delivered_at FROM kw_orders WHERE id = ?");
    ...
    if (!$order) {
        ...
        try {
            kw_restore_order_stock($conn, (int)$order_id);   // quantity_out 되돌림 + lot 삭제
...
<div class="warn">
    이 페이지는 <code>kw_restore_order_stock()</code>을 트랜잭션 안에서 실행한 뒤 <strong>즉시 ROLLBACK</strong> 합니다.
...
<h2>품목별 차감 내역 (kw_order_item_lots)</h2>
<table>
...
<div class="box" style="margin-top:16px">
    ...
    • 증가분이 0이면 <code>kw_order_item_lots</code> 차감기록이 없는 것(pending 등 미차감 주문).<br>
