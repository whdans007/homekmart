...
$_lc_is_staff = kw_is_staff();
$_lc_flash    = kw_get_flash();
...
if ($_lc_is_staff) {
    try {
        ...
        $conn = get_lc_db();
        $_lc_pending_orders   = (int)$conn->query("SELECT COUNT(*) FROM kw_orders WHERE status = 'pending'")->fetch_row()[0];
...
<head>
    ...
    <title><?php echo htmlspecialchars($page_title ?? "KIM'S MALL WAREHOUSE"); ?> - KIM'S MALL WAREHOUSE</title>
