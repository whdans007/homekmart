...
// ── Test 1: kw_is_bundle_unit (Design §4.2) ─────────────────────
echo "[Test 1] kw_is_bundle_unit / kw_valid_unit / kw_normalize_unit\n";
check("kw_is_bundle_unit('BOX')  → true",  kw_is_bundle_unit('BOX') === true);
check("kw_is_bundle_unit('PACK') → true",  kw_is_bundle_unit('PACK') === true);
check("kw_is_bundle_unit('PCS')  → false", kw_is_bundle_unit('PCS') === false);
check("kw_is_bundle_unit(' pack ') → true (공백/소문자)", kw_is_bundle_unit(' pack ') === true);
check("kw_valid_unit('pack') → PACK", kw_valid_unit('pack') === 'PACK');
check("kw_valid_unit('XXX') → PCS(기본)", kw_valid_unit('XXX') === 'PCS');
check("kw_normalize_unit('팩') → PACK", kw_normalize_unit('팩') === 'PACK');
check("kw_normalize_unit('PACK') → PACK", kw_normalize_unit('PACK') === 'PACK');
check("kw_normalize_unit('EA') → PCS", kw_normalize_unit('EA') === 'PCS');
echo "\n";

// ── Test 2: kw_pcs_cost (PACK도 동일 환산) ──────────────────────
echo "[Test 2] kw_pcs_cost (PACK 원가 환산)\n";
check("120/6 = 20.0",   abs(kw_pcs_cost(120, 6) - 20.0) < 0.0001);
check("100/0 = 100 (ppb 보정)", abs(kw_pcs_cost(100, 0) - 100.0) < 0.0001);
echo "\n";

// ── DB 테스트 준비 ──────────────────────────────────────────────
$conn = get_lc_db();

// v22 적용 확인 (kw_inventory.unit ENUM에 PACK 포함)
$col = $conn->query(
    "SELECT COLUMN_TYPE FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'kw_inventory' AND COLUMN_NAME = 'unit'"
)->fetch_assoc();
...
try {
    ...
    $conn->query("INSERT INTO kw_products (name_en, unit, pieces_per_box, is_active) VALUES ('__TEST_PACK__', 'PACK', 6, 1)");
    $pid = $conn->insert_id;

    // PACK 5개 입고 (PACK원가 120 → PCS원가 20)
    $today = date('Y-m-d');
    $pcs_cost = kw_pcs_cost(120.0, 6);
    $st = $conn->prepare(
        "INSERT INTO kw_inbound (inbound_date, product_id, lot_number, expiry_date, quantity, inbound_unit, pieces_per_box, cost_price, cost_price_pcs)
    ...
    $st = $conn->prepare(
        "INSERT INTO kw_inventory (inbound_id, product_id, unit, lot_number, expiry_date, quantity_in, quantity_out)
    ...
    echo "[Test 3] PACK 5 입고 → kw_get_stock_by_unit (3키)\n";
    $stock = kw_get_stock_by_unit($conn, $pid);
    check("BOX=0, PACK=5, PCS=0", $stock['BOX'] === 0 && $stock['PACK'] === 5 && $stock['PCS'] === 0, json_encode($stock));
    check("kw_format_stock = '5 PACK'", kw_format_stock($stock) === '5 PACK', kw_format_stock($stock));
    echo "\n";

    // ── Test 4: PACK 개봉 — 1팩 개봉, 파손 1 (Design §4.4) ──────────
    echo "[Test 4] 1팩(ppb6) 개봉, 파손 1 → PACK 4 + PCS 5\n";
    $res = kw_execute_box_break($conn, $pack_lot_id, 1, 1, 'L1 PACK test');
    ...
    $row = $conn->query(
        "SELECT " . LC_EFFECTIVE_COST_SQL . " AS ec
         FROM kw_inventory i JOIN kw_inbound b ON i.inbound_id = b.id
    ...
    $ded = kw_unit_fifo_ship_allow_negative($conn, $pid, 2, 'PACK');
    $stock = kw_get_stock_by_unit($conn, $pid);
    ...
    $prev = kw_unit_fefo_preview_allow_negative($conn, $pid, 20, 'PCS');
    ...
    $before = kw_get_stock_by_unit($conn, $pid);
    $res = kw_execute_box_break($conn, $pack_lot_id, 99, 0);
    check("잔여 초과 개봉 거부", $res['success'] === false, json_encode($res));
    $res = kw_execute_box_break($conn, $pack_lot_id, 1, 7); // 파손 > 1팩×6
    check("파손 수량 초과(>packs×ppb) 거부", $res['success'] === false, json_encode($res));
    $res = kw_execute_box_break($conn, $pcs_lot_id, 1, 0);
    check("PCS lot 개봉 거부 (최소 단위)", $res['success'] === false, json_encode($res));
    $after = kw_get_stock_by_unit($conn, $pid);
    ...
    $res = kw_execute_box_break($conn, $pack_lot_id, 1, 6, '전량 파손 테스트');
