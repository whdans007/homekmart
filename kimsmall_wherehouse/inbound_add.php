...
try {
    ...
    $brands     = $conn->query("SELECT id, name_en, name_ko FROM kw_brands ORDER BY name_en ASC")->fetch_all(MYSQLI_ASSOC);
    $categories = $conn->query("SELECT id, name_en, name_ko FROM kw_categories ORDER BY name_en ASC")->fetch_all(MYSQLI_ASSOC);

    // 기존 배치가 있으면 날짜·공급업체·기존 아이템 미리 채우기
    if ($existing_batch_id) {
        $st = $conn->prepare("SELECT inbound_date, supplier_id, is_confirmed FROM kw_inbound_batches WHERE id = ?");
        ...
        if ($existing_batch && $existing_batch['is_confirmed']) {
            $conn->close();
            kw_set_flash('error', 'This inbound record is locked and items cannot be added.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($product_ids as $i => $pid) {
        ...
        $unit = kw_valid_unit($units[$i] ?? '', LC_UNIT_PCS);
        // Design Ref: inbound-ppb-override §3.1 — 행별 ppb 오버라이드, 마스터값은 폴백
        $ppb  = kw_resolve_row_ppb($pieces_per_box_inputs[$i] ?? null, $ppb_map[$pid] ?? 1);
        // Design Ref: pack-unit §4 — 묶음(BOX/PACK) 입고는 낱개원가 환산, PCS는 그대로
        $cost_price_pcs = kw_is_bundle_unit($unit) ? kw_pcs_cost($final_price, $ppb) : round($final_price, 4);
    ...
    if (empty($errors)) {
        try {
            ...
            if ($existing_batch_id) {
                ...
                $lock_check = $conn->prepare("SELECT is_confirmed FROM kw_inbound_batches WHERE id = ?");
                ...
                // 새 배치 생성
                $st = $conn->prepare(
                    "INSERT INTO kw_inbound_batches (inbound_date, supplier_id, created_by) VALUES (?,?,?)"
                );
            ...
            foreach ($valid_items as $item) {
                ...
                if ($sellable_qty > 0) {
                    kw_rebalance_negative_lots($conn, $item['product_id']);
                }

                // Design Ref: inbound-damage-registration.design.md §3.3 — 파손 이력 기록
                if ($item['damaged_qty'] > 0) {
                    $damaged_pcs_equiv = kw_is_bundle_unit($item['unit'])
                        ? $item['damaged_qty'] * $item['pieces_per_box']
                        : $item['damaged_qty'];
                    $cost_loss = round($damaged_pcs_equiv * $item['cost_price_pcs'], 4);

                    $st3 = $conn->prepare(
                        "INSERT INTO kw_inbound_damages (inbound_id, product_id, supplier_id, quantity, unit, cost_loss, reason, created_by)
                         VALUES (?,?,?,?,?,?,?,?)"
                    );
            ...
            kw_set_flash('success', count($valid_items) . ' inbound record(s) registered.');
        ...
        <table id="itemsTable" class="w-full text-sm" style="min-width:960px">
            ...
            <tbody id="itemsBody">
                ...
                <?php
                    ...
                    $ei_is_bundle = kw_is_bundle_unit($ei_unit);
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($errors)):
                    foreach (($product_ids ?? []) as $ri => $rpid):
                        ...
                        if (!$rp) continue;
                        $has_restored = true;
                        $r_unit  = kw_valid_unit($units[$ri] ?? '', LC_UNIT_PCS);
                        ...
                        $r_ppb   = kw_resolve_row_ppb($pieces_per_box_inputs[$ri] ?? null, $rp['pieces_per_box'] ?? 1);
                        $r_req_expiry = !empty($rp['requires_expiry']) ? 1 : 0;
                        $r_is_bundle = kw_is_bundle_unit($r_unit); // Design Ref: pack-unit §4 — BOX/PACK 공통 bundle 처리
                ...
                <div class="border-b border-gray-100 pb-5">
                    <div class="grid grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="barcode_logistics" id="inbReg_barcode_logistics" autocomplete="off" placeholder="KIM'S MALL WAREHOUSE Code"
                                   class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-teal-500">
                        </div>
                    </div>
                </div>
...
window.saveSupplier = function() {
    ...
    var LC_BASE_S = '<?php echo LC_BASE; ?>';
    var fd = new FormData();
    fd.append('csrf_token',     '<?php echo htmlspecialchars(kw_csrf_token()); ?>');
    fd.append('name',           name);
...
(function() {
    ...
    var inbRegQuickType  = '';
    var LC_BASE          = '<?php echo LC_BASE; ?>';
    var csrfToken        = '<?php echo htmlspecialchars(kw_csrf_token()); ?>';
