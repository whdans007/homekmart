...
try {
    ...
    if ($form['is_confirmed']) {
        $conn->close();
        kw_set_flash('error', 'This inbound record is locked and cannot be edited.');
        header('Location: ' . $back_url); exit;
    }

    // 출고 여부 확인
    $st = $conn->prepare("SELECT quantity_out FROM kw_inventory WHERE inbound_id = ?");
    $st->bind_param('i', $id); $st->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $form['inbound_unit']   = kw_valid_unit($_POST['inbound_unit'] ?? '', LC_UNIT_PCS);
    ...
    $form['cost_price_pcs'] = kw_is_bundle_unit($form['inbound_unit'])
        ? kw_pcs_cost((float)$form['cost_price'], $form['pieces_per_box'])
    ...
    if (empty($errors)) {
        try {
            ...
            $st = $conn->prepare(
                "UPDATE kw_inbound SET inbound_date=?, product_id=?, lot_number=?, expiry_date=?,
            ...
            // FR-13: lot 단위도 일관 갱신
            $st = $conn->prepare(
                "UPDATE kw_inventory SET product_id=?, unit=?, lot_number=?, expiry_date=?, storage_location=?, quantity_in=?
                 WHERE inbound_id=?"
            );
