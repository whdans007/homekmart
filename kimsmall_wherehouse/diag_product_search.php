 ...
 *   3) kw_inbound.batch_id 가 kw_inbound_batches 에 실제로 존재 (INNER JOIN)
 *
 * logistics/inventory.php 기본 목록 조건:
 *   1) is_active 조건 없음 (비활성 상품도 노출)
 *   2) i.quantity_remain <> 0 (음수 lot도 노출)
 *   3) kw_inbound_batches 조인 없음
 *
 * 사용법: /kimsmall_wherehouse/diag_product_search.php?q=화장지
        ...
        $st = $conn->prepare(
            "SELECT p.id, p.name_en, p.name_ko, p.is_active,
                    ...
                    (SELECT COALESCE(SUM(i.quantity_remain), 0)
                       FROM kw_inventory i JOIN kw_inbound ib ON i.inbound_id = ib.id
                    ...
                    (SELECT COALESCE(SUM(i.quantity_remain), 0)
                       ...
                       JOIN kw_inbound_batches bat ON ib.batch_id = bat.id
                       WHERE i.product_id = p.id AND i.quantity_remain > 0) AS order_php_stock,
                    -- batch_id 가 kw_inbound_batches 에 없는(고아) inbound 건수
                    (SELECT COUNT(*) FROM kw_inbound ib2
                       LEFT JOIN kw_inbound_batches bat2 ON ib2.batch_id = bat2.id
        ...
        foreach ($rows as &$r) {
            ...
            if ((float)$r['order_php_stock'] <= 0 && (float)$r['inventory_php_stock'] > 0) {
                if ((int)$r['orphan_batch_inbound_cnt'] > 0) {
                    $reasons[] = 'batch_id 고아 레코드 ' . $r['orphan_batch_inbound_cnt'] . '건 (kw_inbound_batches 에 없음 — order.php INNER JOIN에서 누락)';
                } else {
