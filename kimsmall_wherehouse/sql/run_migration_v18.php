<?php
/**
 * KIM'S MALL 창고 DB 마이그레이션 v18
 * BOX/PCS 단위 분리 재고 관리
 * Design Ref: box-pcs-unit.design.md §3.1
 *  - kw_inbound: inbound_unit, pieces_per_box, cost_price_pcs
 *  - kw_inventory: unit (+ idx_product_unit)
 *  - kw_order_items: order_unit, pieces_per_box
 *  - kw_box_breaks 테이블 신규
 *  - 기존 데이터 라벨링 (수량 무변경 — Plan FR-10, SC-8)
 * 접속: http://서버주소/kimsmall_wherehouse/sql/run_migration_v18.php
 * 주의: 실행 후 이 파일은 삭제하세요!
 */
...
// ── 1) kw_inbound 컬럼 ──────────────────────────────────────────
if (v18_column_exists($conn, 'kw_inbound', 'inbound_unit')) {
    $steps[] = ['SKIP', 'kw_inbound.inbound_unit 이미 존재'];
} else {
    v18_run($conn, $steps, 'kw_inbound: inbound_unit/pieces_per_box/cost_price_pcs 추가',
...
// ── 2) kw_inventory 컬럼 + 인덱스 ───────────────────────────────
$inv_col_added = false;
if (v18_column_exists($conn, 'kw_inventory', 'unit')) {
    $steps[] = ['SKIP', 'kw_inventory.unit 이미 존재'];
} else {
    $inv_col_added = v18_run($conn, $steps, 'kw_inventory: unit 추가',
        "ALTER TABLE kw_inventory
            ADD COLUMN unit ENUM('BOX','PCS') NOT NULL DEFAULT 'PCS' COMMENT 'lot 단위' AFTER product_id");
}
if (v18_index_exists($conn, 'kw_inventory', 'idx_product_unit')) {
    $steps[] = ['SKIP', 'idx_product_unit 인덱스 이미 존재'];
} else {
    v18_run($conn, $steps, 'kw_inventory: idx_product_unit 인덱스 추가',
        "ALTER TABLE kw_inventory ADD INDEX idx_product_unit (product_id, unit)");
}

// ── 3) kw_order_items 컬럼 ──────────────────────────────────────
if (v18_column_exists($conn, 'kw_order_items', 'order_unit')) {
    $steps[] = ['SKIP', 'kw_order_items.order_unit 이미 존재'];
} else {
    v18_run($conn, $steps, 'kw_order_items: order_unit/pieces_per_box 추가',
...
// ── 4) kw_box_breaks 테이블 ─────────────────────────────────────
if (v18_table_exists($conn, 'kw_box_breaks')) {
    $steps[] = ['SKIP', 'kw_box_breaks 테이블 이미 존재'];
} else {
    v18_run($conn, $steps, 'kw_box_breaks 테이블 생성',
        "CREATE TABLE kw_box_breaks (
...
if (v18_run($conn, $steps, 'kw_inventory 단위 라벨링',
...
v18_run($conn, $steps, 'kw_inbound 단위/ppb/PCS원가 라벨링',
...
v18_run($conn, $steps, 'kw_order_items 단위/ppb 라벨링',
...
$report = $conn->query(
    "SELECT unit, COUNT(*) AS lots, COALESCE(SUM(quantity_in - quantity_out),0) AS remain
     FROM kw_inventory GROUP BY unit"
)->fetch_all(MYSQLI_ASSOC);
foreach ($report as $r) {
    $steps[] = ['INFO', "재고 라벨링 결과: {$r['unit']} lot {$r['lots']}건, 잔여 수량 {$r['remain']}"];
}
$zero_pcs = $conn->query(
    "SELECT COUNT(*) FROM kw_inbound WHERE cost_price > 0 AND cost_price_pcs = 0"
)->fetch_row()[0];
