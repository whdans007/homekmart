 ...
 *
 * 원인: kw_fifo_ship_allow_negative()가 재고 부족분을 "그 시점에 존재하던 최근 LOT"에
 *       음수로 기록하는데(당시엔 LOT이 하나뿐이라 자기 자신에 기록됨), 이후 새 LOT이 입고돼도
 *       과거 음수가 자동으로 재조정되지 않던 문제. (재발 방지: lib/inventory_helper.php
 *       kw_rebalance_negative_lots() 추가 + inbound_add.php 신규 입고 시 자동 호출)
 *
 * 처리 방식: kw_rebalance_negative_lots()와 동일한 로직으로, 음수 LOT의 quantity_out을
 ...
 *
 * 접속: http://main.homekmart.net/kimsmall_wherehouse/sql/fix_negative_lot_88004464.php
...
$st = $conn->prepare(
    "SELECT id, name_en, name_ko FROM kw_products
...
if (!$product) {
    ...
    $before_lots = $conn->query(
        "SELECT id, expiry_date, quantity_in, quantity_out, quantity_remain
         FROM kw_inventory WHERE product_id = $product_id ORDER BY expiry_date ASC, id ASC"
    ...
    $before_totals = $conn->query(
        "SELECT COALESCE(SUM(quantity_in),0) tin, COALESCE(SUM(quantity_out),0) tout,
         ...
         FROM kw_inventory WHERE product_id = $product_id"
    )->fetch_assoc();
    ...
    if ((int)$before_totals['neg_cnt'] === 0) {
        ...
        $ok = true;
        try {
            kw_rebalance_negative_lots($conn, $product_id);
        } catch (Throwable $e) {
            $ok = false;
