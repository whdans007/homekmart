...
-- 1. 브랜드 테이블
CREATE TABLE IF NOT EXISTS kw_brands (
...
-- 2. 카테고리 테이블 (대/소분류 지원)
CREATE TABLE IF NOT EXISTS kw_categories (
    id         INT AUTO_INCREMENT PRIMARY KEY,
...
-- 3. kw_products 컬럼 변경 (ALTER TABLE)
-- 3-1. 영문명으로 컬럼 이름 변경
...
UPDATE kw_products SET barcode_unit = barcode WHERE barcode IS NOT NULL AND barcode != '';
ALTER TABLE kw_products DROP COLUMN barcode;
...
ALTER TABLE kw_products DROP COLUMN category;
...
SELECT 'kw_products v2 마이그레이션 완료' AS result;
