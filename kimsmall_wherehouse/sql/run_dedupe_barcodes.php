...
// 스캔:  http://main.homekmart.net/kimsmall_wherehouse/sql/run_dedupe_barcodes.php
// 동작:
...
function product_barcodes(mysqli $conn, int $id): array {
    $st = $conn->prepare("SELECT barcode_unit, barcode_box, barcode_logistics FROM kw_products WHERE id = ?");
...
function bc_holders(mysqli $conn, string $bc): int {
    ...
    $st = $conn->prepare("SELECT COUNT(*) FROM kw_products WHERE barcode_unit = ? OR barcode_box = ? OR barcode_logistics = ?");
...
function barcode_still_used(mysqli $conn, string $bc, int $selfId, array $deleteSet): bool {
    ...
    $sql = "SELECT COUNT(*) FROM kw_products
...
try {
    ...
    $dupSql = "SELECT bc, COUNT(DISTINCT id) AS pc
               FROM (
                   SELECT id, barcode_unit AS bc FROM kw_products WHERE barcode_unit IS NOT NULL AND barcode_unit <> ''
                   UNION ALL
                   SELECT id, barcode_box       FROM kw_products WHERE barcode_box IS NOT NULL AND barcode_box <> ''
                   UNION ALL
                   SELECT id, barcode_logistics FROM kw_products WHERE barcode_logistics IS NOT NULL AND barcode_logistics <> ''
               ) t
               GROUP BY bc
