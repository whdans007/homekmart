<?php
$page_title = "Inventory Status - KIM'S MALL WAREHOUSE";
...
try {
    ...
    if ($filter === 'out') {
        // 재고 0 상품 (min_stock 설정된 상품 중 현재고 0 이하) — kw_inventory에 재고 lot이 없어도
        // 노출되도록 kw_products 기준 LEFT JOIN (다른 필터는 kw_inventory INNER JOIN 기반이라
    ...
    <div class="overflow-auto flex-1 min-h-0">
        <table class="w-full text-sm">
            ...
            <?php foreach ($list as $row):
                ...
                $stockDisplay = kw_format_stock([LC_UNIT_BOX => $boxStock, LC_UNIT_PACK => $packStock, LC_UNIT_PCS => $pcsStock]);
