<?php
$page_title = "Edit Product - KIM'S MALL WAREHOUSE";
...
try {
    ...
    $st->close();
    if (!$form) { $conn->close(); kw_set_flash('error','Product not found.'); header('Location: ' . $list_url); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete') {
    ...
    try {
        ...
        $st = $conn->prepare(
            "SELECT
               (SELECT COUNT(*) FROM kw_inbound    WHERE product_id = ?) +
               (SELECT COUNT(*) FROM kw_inventory  WHERE product_id = ?) +
               (SELECT COUNT(*) FROM kw_order_items WHERE product_id = ?) AS cnt"
        );
        ...
        if ($cnt > 0) {
            $conn->close();
            kw_set_flash('error', 'Cannot delete products with inbound/inventory/order history. Use deactivation instead.');
            header('Location: ' . LC_BASE . '/product_edit.php?id=' . $id . $ret_q . $back_q);
            exit;
        }

        $st = $conn->prepare("DELETE FROM kw_products WHERE id = ?");
        $st->bind_param('i', $id);
        $st->execute();
        $conn->close();
        kw_set_flash('success', 'Product deleted successfully.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if (isset($_POST['remove_image']) && $_POST['remove_image'] === '1') {
            if (!empty($old_form['image_path'])) kw_delete_product_image($old_form['image_path']);
            $form['image_path'] = null;
        }
        if (isset($_FILES['image']) && ($_FILES['image']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
            $img_err = '';
            $new_img = kw_handle_product_image_upload($_FILES['image'], $img_err);
            if ($new_img !== null) {
                if (!empty($old_form['image_path'])) kw_delete_product_image($old_form['image_path']);
    ...
    if (empty($errors)) {
        try {
            ...
            $sh = $conn->prepare(
                "INSERT INTO kw_product_history
