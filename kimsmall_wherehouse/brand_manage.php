<?php
$page_title = "Brand Management - KIM'S MALL WAREHOUSE";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if ($name_en !== '') {
            ...
            kw_set_flash('success', "Brand '{$name_en}' registered successfully");
        }
    } elseif ($action === 'edit') {
        ...
        $name_ko = trim($_POST['name_ko'] ?? '') ?: null;
        if ($bid > 0 && $name_en !== '') {
            $st = $conn->prepare("UPDATE kw_brands SET name_en = ?, name_ko = ? WHERE id = ?");
            $st->bind_param('ssi', $name_en, $name_ko, $bid); $st->execute(); $st->close();
            kw_set_flash('success', "Brand '{$name_en}' updated successfully");
        }
    } elseif ($action === 'delete') {
        $bid = (int)($_POST['brand_id'] ?? 0);
        $st = $conn->prepare("DELETE FROM kw_brands WHERE id = ?");
...
try {
    ...
    $where = ''; $params = []; $types = '';
    if ($search !== '') {
        $where = "WHERE (b.name_en LIKE ? OR b.name_ko LIKE ?)";
        $params = ["%$search%", "%$search%"]; $types = 'ss';
    }

    $cnt = $conn->prepare("SELECT COUNT(*) FROM kw_brands b $where");
    ...
    $st->execute();
    $brands = $st->get_result()->fetch_all(MYSQLI_ASSOC); $st->close();
    $conn->close();
} catch (Exception $e) {
    $db_error = $e->getMessage();
    $brands = []; $total = 0; $total_pages = 1;
}
?>

<style>
/* brand_manage 전용: main 스크롤 비활성화, flex 레이아웃으로 뷰포트 꽉 채움 */
...
<div class="bg-white rounded-lg border border-gray-200 px-3 py-2 shrink-0">
    <form method="post" id="brandForm" onsubmit="return prepareBrandSubmit()">
...
<div class="bg-white rounded-lg border border-gray-200 overflow-hidden flex flex-col flex-1 min-h-0">
    ...
    <div class="overflow-auto flex-1 min-h-0">
        <table class="w-full text-sm">
            ...
            <tr class="hover:bg-gray-50 <?php echo $b['product_count'] > 0 ? 'cursor-pointer' : ''; ?>"
                <?php if ($b['product_count'] > 0): ?>onclick="openProductsModal(<?php echo $b['id']; ?>, <?php echo htmlspecialchars(json_encode($b['name_en']), ENT_QUOTES); ?>, <?php echo (int)$b['product_count']; ?>)"<?php endif; ?>>
                <td class="px-4 py-3 font-medium text-gray-900"><?php echo htmlspecialchars($b['name_en']); ?></td>
                <td class="px-4 py-3 text-gray-600 text-xs"><?php echo $b['name_ko'] ? htmlspecialchars($b['name_ko']) : '-'; ?></td>
                <td class="px-4 py-3 text-center text-gray-600 text-xs"><?php echo (int)$b['product_count']; ?></td>
...
(function() {
    ...
    window.romanizeBrandName = function() {
        var koInput = document.getElementById('brandKoInput');
        var enInput = document.getElementById('brandEnInput');
    ...
    window.prepareBrandSubmit = function() {
        var en = document.getElementById('brandEnInput').value.trim();
        if (!en) { document.getElementById('brandEnInput').focus(); return false; }
        return true;
    };
