...
$page_title = "Request Detail - KIM'S MALL WAREHOUSE";
...
$st = $conn->prepare(
    "SELECT r.*, s.name AS store_name FROM kw_store_requests r JOIN stores s ON r.store_id = s.id WHERE r.id = ?"
);
$st->bind_param('i', $id);
$st->execute();
...
if (!$request) {
    $conn->close();
    kw_set_flash('error', 'Request not found.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Plan SC-5: KIM'S MALL 창고 측 댓글(답변) 작성
    if ($action === 'comment') {
        ...
        if ($content !== '') {
            ...
            $st = $conn->prepare(
                "INSERT INTO kw_store_request_comments (request_id, author_side, author_id, author_name, content) VALUES (?, 'logistics', ?, ?, ?)"
            );
            $st->bind_param('iiss', $id, $uid, $name, $content);
            $st->execute();
            $st->close();
        }
    // Plan SC-6: KIM'S MALL 창고만 상태 변경 가능
    } elseif ($action === 'status') {
        $new_status = $_POST['status'] ?? '';
        if (in_array($new_status, ['pending', 'in_progress', 'done'], true)) {
            $st = $conn->prepare("UPDATE kw_store_requests SET status = ? WHERE id = ?");
            $st->bind_param('si', $new_status, $id);
            $st->execute();
            $st->close();
            kw_set_flash('success', 'Status updated.');
        } else {
            kw_set_flash('error', 'Invalid status.');
        }
    }
...
$st = $conn->prepare("SELECT * FROM kw_store_request_comments WHERE request_id = ? ORDER BY created_at ASC");
$st->bind_param('i', $id);
$st->execute();
            ...
            <div class="rounded-lg p-3 <?php echo $c['author_side'] === 'logistics' ? 'bg-blue-50 border border-blue-100' : 'bg-teal-50 border border-teal-100'; ?>">
                <div class="flex items-center justify-between mb-1">
                    <span class="text-xs font-semibold <?php echo $c['author_side'] === 'logistics' ? 'text-blue-700' : 'text-teal-700'; ?>">
                        <?php echo $c['author_side'] === 'logistics' ? "KIM'S MALL WAREHOUSE" : 'Store'; ?> · <?php echo htmlspecialchars($c['author_name']); ?>
                    </span>
