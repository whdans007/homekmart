...
try {
    ...
    if ($search) {
        $where_clauses[] = "(p.name_ko LIKE ? OR p.name_en LIKE ? OR p.sku LIKE ?)";
        $search_param = "%$search%";
        $params[] = $search_param;
        $params[] = $search_param;
        $params[] = $search_param;
    ...
    // 파라미터 바인딩 (LIMIT/OFFSET는 정수로 명시)
    $bind_params = $params;
    foreach ($bind_params as $key => $value) {
        $stmt->bindValue($key + 1, $value);
    }
    $stmt->bindValue(count($bind_params) + 1, $pagination['limit'], PDO::PARAM_INT);
    $stmt->bindValue(count($bind_params) + 2, $pagination['offset'], PDO::PARAM_INT);
