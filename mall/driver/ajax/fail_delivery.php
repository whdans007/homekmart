<?php
/**
 * POST mall/driver/ajax/fail_delivery.php
 * Design Ref: mall-delivery-dispatch.design.md §4.1 — assigned/delivering → delivery_failed
 */
...
if ($order_id <= 0 || $reason === '') {
    json_error('VALIDATION_ERROR', '실패 사유를 입력해주세요');
}

$error_messages = [
    'NOT_ASSIGNED_TO_YOU' => '본인에게 배정된 주문이 아닙니다',
    'INVALID_STATE_TRANSITION' => '처리할 수 없는 상태입니다',
    'SERVER_ERROR' => '처리 중 오류가 발생했습니다',
];

$result = mall_delivery_fail($driver['id'], $order_id, $reason);
if (!$result['success']) {
