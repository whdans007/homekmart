<?php
/**
 * 홈 레이아웃 빌더 재도입 DB 마이그레이션 (mall_home_sections 테이블 재생성)
 * 접속: http://서버주소/sunset/sql/run_create_mall_home_sections_migration.php
 *
 * 이전에 sql/run_drop_mall_home_sections_migration.php로 폐기했던 홈 레이아웃 빌더를
 * 다시 도입하면서 테이블을 재생성한다. 초안(draft)/발행(published) 컬럼까지 포함한 최종
 * 스키마로 한 번에 만든다. 이미 테이블이 있으면 안전하게 SKIP한다.
 *
 * 주의: 실행 후 이 파일을 삭제하세요.
 */
...
<head>
    ...
    <title>홈 레이아웃 빌더 재도입 DB 마이그레이션</title>
    <style>
...
<body>
<div class="card">
    <h1>🚀 홈 레이아웃 빌더 재도입 DB 마이그레이션</h1>
    <p class="sub">mall_home_sections 테이블 생성 · DB: <strong><?php echo DB_NAME; ?></strong></p>
</div>
