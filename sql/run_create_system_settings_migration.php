<?php
/**
 * system_settings 테이블 생성 마이그레이션
 * mall/admin/products.php · mall/admin/discount_rules.php의 "기준도매가 마진율" 설정에 필요.
 * 접속: https://homekmart.net/sql/run_create_system_settings_migration.php
 *
 * 이미 테이블이 존재하면 SKIP 처리되어 여러 번 실행해도 안전하다(idempotent).
 *
 * 주의: 실행 후 이 파일을 삭제하세요.
 */
...
$schema_file = __DIR__ . '/create_system_settings.sql';
$create_sql = trim(preg_replace('/--.*$/m', '', file_get_contents($schema_file)));
...
<head>
    ...
    <title>system_settings 마이그레이션</title>
    <style>
...
<body>
<div class="card">
    <h1>🛠️ system_settings 테이블 마이그레이션</h1>
    <p class="sub">DB: <strong><?php echo DB_NAME; ?></strong></p>
</div>

<?php if (!$ran && $already_exists): ?>
<div class="card">
    <div class="skip-box">➖ <code>system_settings</code> 테이블이 이미 존재합니다. 실행할 필요가 없습니다.</div>
</div>
<?php elseif ($ran): ?>
<div class="card">
    <?php if ($status === 'OK'): ?>
    <div class="success-box">✅ <code>system_settings</code> 테이블 생성 완료!</div>
    <p class="sub">⚠️ 보안을 위해 이 파일을 삭제하세요.</p>
    <?php elseif ($status === 'SKIP'): ?>
    <div class="skip-box">➖ 이미 존재하여 건너뛰었습니다.</div>
    <?php else: ?>
    <div class="err-box">❌ 생성 실패: <?php echo htmlspecialchars($error_msg); ?></div>
    <?php endif; ?>
</div>
<?php else: ?>
<div class="card">
    <div class="warn">
        ⚠️ <code>system_settings</code> 테이블(key-value 설정 저장용)을 새로 생성합니다. 다른 테이블에는 영향을 주지 않습니다.
    </div>
    <form method="post">
        <input type="hidden" name="action" value="run">
        <button type="submit" onclick="return confirm('system_settings 테이블을 생성하시겠습니까?')">
            마이그레이션 실행
        </button>
    </form>
</div>
