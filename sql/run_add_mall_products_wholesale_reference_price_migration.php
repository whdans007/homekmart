<?php
/**
 * mall_products 테이블에 wholesale_reference_price 컬럼 추가 마이그레이션
 * mall/admin/products.php 큐레이션 테이블에서 "기준도매가"를 직접 수정할 수 있게 하는 데 필요.
 * 접속: https://homekmart.net/sql/run_add_mall_products_wholesale_reference_price_migration.php
 *
...
$check = $conn->query("SHOW COLUMNS FROM mall_products LIKE 'wholesale_reference_price'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'run') {
    ...
    if ($already_exists) {
        $status = 'SKIP';
    } elseif ($conn->query("ALTER TABLE `mall_products` ADD COLUMN `wholesale_reference_price` DECIMAL(10,2) NULL DEFAULT NULL AFTER `display_name_en`")) {
        $status = 'OK';
    } else {
        $status = 'ERROR';
...
<head>
    ...
    <title>mall_products.wholesale_reference_price 마이그레이션</title>
    <style>
...
<body>
<div class="card">
    <h1>🛠️ mall_products.wholesale_reference_price 컬럼 마이그레이션</h1>
    <p class="sub">DB: <strong><?php echo DB_NAME; ?></strong></p>
</div>

<?php if (!$ran && $already_exists): ?>
<div class="card">
    <div class="skip-box">➖ <code>mall_products.wholesale_reference_price</code> 컬럼이 이미 존재합니다. 실행할 필요가 없습니다.</div>
</div>
<?php elseif ($ran): ?>
<div class="card">
    <?php if ($status === 'OK'): ?>
    <div class="success-box">✅ <code>wholesale_reference_price</code> 컬럼 추가 완료!</div>
...
<div class="card">
    <div class="warn">
        ⚠️ <code>mall_products</code> 테이블에 <code>wholesale_reference_price</code> 컬럼(NULL 허용)을 추가합니다. 기존 데이터에는 영향을 주지 않습니다.
    </div>
    <form method="post">
        <input type="hidden" name="action" value="run">
        <button type="submit" onclick="return confirm('mall_products 테이블에 wholesale_reference_price 컬럼을 추가하시겠습니까?')">
            마이그레이션 실행
        </button>
    </form>
</div>
