<?php
/**
 * mall_orders 배송비(shipping_fee) 컬럼 추가 DB 마이그레이션
 * 접속: http://서버주소/sunset/sql/run_add_mall_orders_shipping_fee_migration.php
 *
 * - shipping_fee: 주문 확정 시점에 계산해 저장하는 배송비 스냅샷(주문 이후 배송비 정책이
 *   바뀌어도 과거 주문 금액이 바뀌면 안 되므로 order_items처럼 스냅샷 방식으로 저장한다).
 * - total_amount는 이제 subtotal - discount_amount + shipping_fee 이다(기존 주문은 shipping_fee=0으로
 *   채워지므로 total_amount를 다시 계산하지 않는다 — 과거 주문 금액을 건드리지 않기 위함).
...
$col_check = $conn->query("SHOW COLUMNS FROM mall_orders LIKE 'shipping_fee'");
if ($col_check && $col_check->num_rows > 0) {
    $steps[] = ['SKIP', 'shipping_fee 컬럼이 이미 존재합니다.'];
} else {
    if ($conn->query("ALTER TABLE `mall_orders` ADD COLUMN `shipping_fee` DECIMAL(12,2) NOT NULL DEFAULT 0.00 AFTER `discount_amount`")) {
        $steps[] = ['OK', 'shipping_fee 컬럼 추가 완료 (기존 주문은 0.00으로 채워짐)'];
    } else {
...
<body>
<div class="card">
    <h1>🚀 배송비 컬럼 DB 마이그레이션</h1>
    <p class="sub">mall_orders에 shipping_fee 추가 · DB: <strong><?php echo DB_NAME; ?></strong></p>
</div>
