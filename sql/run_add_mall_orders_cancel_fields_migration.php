<?php
/**
 * mall_orders 취소 사유/시각 컬럼 추가 DB 마이그레이션
 * 접속: http://서버주소/sunset/sql/run_add_mall_orders_cancel_fields_migration.php
 * Design Ref: 관리자가 "고객 요청"으로 주문을 취소할 때 사유를 남기기 위한 컬럼.
...
$col_check = $conn->query("SHOW COLUMNS FROM mall_orders LIKE 'cancel_reason'");
if ($col_check && $col_check->num_rows > 0) {
    $steps[] = ['SKIP', 'cancel_reason 컬럼이 이미 존재합니다.'];
} else {
    if ($conn->query("ALTER TABLE `mall_orders`
            ADD COLUMN `cancel_reason` varchar(255) DEFAULT NULL AFTER `memo`,
            ADD COLUMN `cancelled_at` datetime DEFAULT NULL AFTER `cancel_reason`")) {
        $steps[] = ['OK', 'cancel_reason/cancelled_at 컬럼 추가 완료'];
    } else {
...
<body>
<div class="card">
    <h1>🚀 주문취소 컬럼 DB 마이그레이션</h1>
    <p class="sub">mall_orders에 cancel_reason/cancelled_at 추가 · DB: <strong><?php echo DB_NAME; ?></strong></p>
</div>
