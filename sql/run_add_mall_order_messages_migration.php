<?php
/**
 * mall_order_messages(주문톡) 테이블 신설 DB 마이그레이션
 * 접속: http://서버주소/sunset/sql/run_add_mall_order_messages_migration.php
 * Design Ref: mall-order-chat.design.md §3.1
 *
...
if ($table_check && $table_check->num_rows > 0) {
    $steps[] = ['SKIP', 'mall_order_messages 테이블이 이미 존재합니다.'];
} else {
    $sql = "CREATE TABLE `mall_order_messages` (
    ...
    if ($conn->query($sql)) {
        $steps[] = ['OK', 'mall_order_messages 테이블 생성 완료'];
    } else {
...
<body>
<div class="card">
    <h1>🚀 주문톡 테이블 DB 마이그레이션</h1>
    <p class="sub">mall_order_messages 신설 · DB: <strong><?php echo DB_NAME; ?></strong></p>
</div>
