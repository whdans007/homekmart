<?php
/**
 * 몰 상품 "품절 처리" 수동 플래그 DB 마이그레이션
 * 접속: http://서버주소/sunset/sql/run_add_mall_products_sold_out_flag_migration.php
 *
 * mall_products에 is_sold_out 컬럼을 추가한다. 실제 재고(inventory.quantity)는 발주/이동/유통기한 로트로
 * 관리되어 복잡하므로 건드리지 않고, 몰 화면에서만 강제로 품절로 보이게 하는 별도 스위치를 둔다
 * (mall/admin/products.php의 "품절 처리" 체크박스, mall/lib/pricing.php::mall_get_stock_quantity()).
...
$col_check = $conn->query("SHOW COLUMNS FROM mall_products LIKE 'is_sold_out'");
if ($col_check && $col_check->num_rows > 0) {
    $steps[] = ['SKIP', 'is_sold_out 컬럼이 이미 존재합니다.'];
} else {
    if ($conn->query("ALTER TABLE `mall_products` ADD COLUMN `is_sold_out` TINYINT(1) NOT NULL DEFAULT 0 AFTER `is_active`")) {
        $steps[] = ['OK', 'is_sold_out 컬럼 추가 완료'];
    } else {
...
<head>
    ...
    <title>품절 처리 플래그 DB 마이그레이션</title>
    <style>
...
<body>
<div class="card">
    <h1>🚀 품절 처리 플래그 DB 마이그레이션</h1>
    <p class="sub">mall_products에 is_sold_out 추가 · DB: <strong><?php echo DB_NAME; ?></strong></p>
</div>
