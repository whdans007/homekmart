<?php
/**
 * mall_fresh_products에 고정 카테고리(과일/채소/정육/수산) 컬럼 추가
 * 기존 categories 테이블을 참조하지 않는다 — 데이터가 어떻게 구성되어 있든 항상 4개로 고정.
 *
 * 접속: https://main.homekmart.net/sql/migrations/run_add_fresh_products_category.php
...
$check = $conn->query("SHOW COLUMNS FROM mall_fresh_products LIKE 'fresh_category'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'run') {
    ...
    if ($already_exists) {
        $status = 'SKIP';
    } elseif ($conn->query("ALTER TABLE `mall_fresh_products` ADD COLUMN `fresh_category` ENUM('fruit','vegetable','meat','seafood') NOT NULL DEFAULT 'fruit' COMMENT '고정 분류: 과일/채소/정육/수산' AFTER `name_en`")) {
        $status = 'OK';
    } else {
        $status = 'ERROR';
...
<head>
    ...
    <title>mall_fresh_products.fresh_category 마이그레이션</title>
    <style>
...
<body>
<div class="card">
    <h1>🛠️ mall_fresh_products.fresh_category 마이그레이션</h1>
    <p class="sub">DB: <strong><?php echo DB_NAME; ?></strong></p>
</div>

<?php if (!$ran && $already_exists): ?>
<div class="card">
    <div class="skip-box">➖ <code>mall_fresh_products.fresh_category</code> 컬럼이 이미 존재합니다. 실행할 필요가 없습니다.</div>
</div>
<?php elseif ($ran): ?>
<div class="card">
    <?php if ($status === 'OK'): ?>
    <div class="success-box">✅ <code>fresh_category</code> 컬럼 추가 완료! (기본값 fruit)</div>
...
<div class="card">
    <div class="warn">
        ⚠️ <code>mall_fresh_products</code> 테이블에 <code>fresh_category</code> ENUM('fruit','vegetable','meat','seafood') 컬럼(기본값 fruit)을 추가합니다. 기존 categories 테이블과는 무관한 고정 분류입니다.
    </div>
    <form method="post">
        <input type="hidden" name="action" value="run">
        <button type="submit" onclick="return confirm('fresh_category 컬럼을 추가하시겠습니까?')">
            마이그레이션 실행
        </button>
    </form>
</div>
