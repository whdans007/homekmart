<?php
/**
 * mall_fresh_products에 판매 방식(낱개/저울) 구분 컬럼 추가
 * 설계 근거: mall-fresh-products 기능 확장 (2026-09-05 사용자 요청)
 * - sale_type='piece'(낱개/정가): price_per_100g 컬럼을 고정 판매가로 재사용
 * - sale_type='weight'(저울/무게, 기본값): 기존과 동일하게 100g당 판매가로 사용
 *
 * 접속: https://main.homekmart.net/sql/migrations/run_add_fresh_products_sale_type.php
...
$check = $conn->query("SHOW COLUMNS FROM mall_fresh_products LIKE 'sale_type'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'run') {
    ...
    if ($already_exists) {
        $status = 'SKIP';
    } elseif ($conn->query("ALTER TABLE `mall_fresh_products` ADD COLUMN `sale_type` ENUM('piece','weight') NOT NULL DEFAULT 'weight' COMMENT '낱개(정가)/저울(무게) 판매 구분' AFTER `category_id`")) {
        $status = 'OK';
    } else {
        $status = 'ERROR';
...
<head>
    ...
    <title>mall_fresh_products.sale_type 마이그레이션</title>
    <style>
...
<body>
<div class="card">
    <h1>🛠️ mall_fresh_products.sale_type 마이그레이션</h1>
    <p class="sub">DB: <strong><?php echo DB_NAME; ?></strong></p>
</div>

<?php if (!$ran && $already_exists): ?>
<div class="card">
    <div class="skip-box">➖ <code>mall_fresh_products.sale_type</code> 컬럼이 이미 존재합니다. 실행할 필요가 없습니다.</div>
</div>
<?php elseif ($ran): ?>
<div class="card">
    <?php if ($status === 'OK'): ?>
    <div class="success-box">✅ <code>sale_type</code> 컬럼 추가 완료! (기본값 weight)</div>
...
<div class="card">
    <div class="warn">
        ⚠️ <code>mall_fresh_products</code> 테이블에 <code>sale_type</code> ENUM('piece','weight') 컬럼(기본값 weight)을 추가합니다. 기존 데이터는 전부 'weight'로 유지됩니다.
    </div>
    <form method="post">
        <input type="hidden" name="action" value="run">
        <button type="submit" onclick="return confirm('sale_type 컬럼을 추가하시겠습니까?')">
            마이그레이션 실행
        </button>
    </form>
</div>
