<?php
/**
 * mall_fresh_products에 display_name_override / display_name_en_override 컬럼 추가.
 * - 몰 큐레이션 화면(mall/admin/products.php)의 "Display Name"은 쇼핑몰에 노출되는 진열명이고,
 *   admin/fresh_products.php가 보여주는 원본 상품명(name_ko/name_en)과는 별개여야 한다.
 * - 지금까지는 별도 컬럼이 없어 큐레이션에서 진열명을 수정하면 원본 name_ko/name_en 자체가
 *   바뀌어버렸다(cost_price_override 등과 같은 이유로 발생한 동일한 종류의 문제).
 * - NULL이면 원본 name_ko/name_en을 그대로 진열명으로 쓴다.
 *
 * 접속: https://main.homekmart.net/sql/migrations/run_add_mall_fresh_products_display_name_override.php
...
function mfpdno_column_exists(mysqli $conn, string $table, string $column): bool
{
...
$steps = [
    [
        'label' => 'display_name_override 컬럼 추가 (몰 진열명 오버라이드, NULL 허용)',
        'already_done' => mfpdno_column_exists($conn, 'mall_fresh_products', 'display_name_override'),
        'sql' => "ALTER TABLE `mall_fresh_products` ADD COLUMN `display_name_override` VARCHAR(255) NULL DEFAULT NULL COMMENT '몰 진열명(한글) - NULL이면 원본 name_ko 사용' AFTER `name_en`",
    ],
    [
        'label' => 'display_name_en_override 컬럼 추가 (몰 진열명 영문 오버라이드, NULL 허용)',
        'already_done' => mfpdno_column_exists($conn, 'mall_fresh_products', 'display_name_en_override'),
        'sql' => "ALTER TABLE `mall_fresh_products` ADD COLUMN `display_name_en_override` VARCHAR(255) NULL DEFAULT NULL COMMENT '몰 진열명(영문) - NULL이면 원본 name_en 사용' AFTER `display_name_override`",
    ],
];
...
<head>
    ...
    <title>mall_fresh_products display_name_override 마이그레이션</title>
    <style>
...
<body>
<div class="card">
    <h1>🛠️ mall_fresh_products display_name_override 마이그레이션</h1>
...
<div class="card">
    <div class="warn">
        ⚠️ 몰 진열명을 원본 상품명과 분리해서 저장할 수 있는 오버라이드 컬럼을 추가합니다.
    </div>
    <ul class="steps">
