<?php
/**
 * mall_members 영문 이름(english_name) 컬럼 추가 DB 마이그레이션
 * 접속: http://서버주소/sunset/sql/run_add_mall_members_english_name_migration.php
 *
 * - english_name: 해외 배송(필리핀 등) 시 현지 배송기사가 알아볼 수 있도록 사용하는 영문 이름.
...
$col_check = $conn->query("SHOW COLUMNS FROM mall_members LIKE 'english_name'");
if ($col_check && $col_check->num_rows > 0) {
    $steps[] = ['SKIP', 'english_name 컬럼이 이미 존재합니다.'];
} else {
    if ($conn->query("ALTER TABLE `mall_members` ADD COLUMN `english_name` VARCHAR(100) NULL DEFAULT NULL AFTER `name`")) {
        $steps[] = ['OK', 'english_name 컬럼 추가 완료'];
    } else {
...
<head>
    ...
    <title>영문 이름 컬럼 DB 마이그레이션</title>
    <style>
...
<body>
<div class="card">
    <h1>🚀 영문 이름 컬럼 DB 마이그레이션</h1>
    <p class="sub">mall_members에 english_name 추가 · DB: <strong><?php echo DB_NAME; ?></strong></p>
</div>
