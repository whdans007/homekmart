<?php
/**
 * mall_order_items 품절 표시(is_sold_out) 컬럼 추가 DB 마이그레이션
 * 접속: http://서버주소/sunset/sql/run_add_mall_order_items_sold_out_migration.php
 *
 * - is_sold_out: 주문 처리 중(피킹) 특정 항목이 품절로 확인되면 그 주문 건에서만 제외 표시하기 위한 플래그.
 *   1이 되면 관리자 주문 상세/피킹슬립/영수증에서 품절로 표시되고, 해당 주문의 subtotal/discount_amount/
 *   shipping_fee/total_amount 재계산 시 이 항목은 제외된다(mall_products.is_sold_out과는 별개 — 그쪽은
 *   몰 전체 진열 기준, 이쪽은 이미 확정된 개별 주문 기준).
...
$col_check = $conn->query("SHOW COLUMNS FROM mall_order_items LIKE 'is_sold_out'");
if ($col_check && $col_check->num_rows > 0) {
    $steps[] = ['SKIP', 'is_sold_out 컬럼이 이미 존재합니다.'];
} else {
    if ($conn->query("ALTER TABLE `mall_order_items` ADD COLUMN `is_sold_out` TINYINT(1) NOT NULL DEFAULT 0 AFTER `line_total`")) {
        $steps[] = ['OK', 'is_sold_out 컬럼 추가 완료 (기존 항목은 0으로 채워짐)'];
    } else {
...
<head>
    ...
    <title>주문항목 품절표시 컬럼 DB 마이그레이션</title>
    <style>
...
<body>
<div class="card">
    <h1>🚀 주문항목 품절표시 컬럼 DB 마이그레이션</h1>
    <p class="sub">mall_order_items에 is_sold_out 추가 · DB: <strong><?php echo DB_NAME; ?></strong></p>
</div>
