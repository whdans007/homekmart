<?php
/**
 * mall_products 테이블에 retail_discount_allowed / wholesale_discount_allowed 컬럼 추가 마이그레이션
 * 기존 discount_allowed(채널 구분 없음)를 대체해, 상품별로 일반(소매) 할인과
 * 도매 할인을 따로 허용/차단할 수 있게 한다. 기존 discount_allowed 컬럼은 남겨두되
 * 더 이상 코드에서 참조하지 않는다.
 * 접속: https://homekmart.net/sql/run_add_mall_products_discount_flags_migration.php
 *
...
$columns_to_add = [
    'retail_discount_allowed' => "ALTER TABLE `mall_products` ADD COLUMN `retail_discount_allowed` TINYINT(1) NOT NULL DEFAULT 1",
    'wholesale_discount_allowed' => "ALTER TABLE `mall_products` ADD COLUMN `wholesale_discount_allowed` TINYINT(1) NOT NULL DEFAULT 1",
];
...
<head>
    ...
    <title>mall_products 할인 플래그 마이그레이션</title>
    <style>
...
<body>
<div class="card">
    <h1>🛠️ mall_products 할인 플래그 마이그레이션</h1>
    <p class="sub">DB: <strong><?php echo DB_NAME; ?></strong> · retail_discount_allowed, wholesale_discount_allowed</p>
</div>

<?php if (!$ran && $already_all_exist): ?>
<div class="card">
    <div class="skip-box">➖ 두 컬럼 모두 이미 존재합니다. 실행할 필요가 없습니다.</div>
</div>
<?php elseif ($ran): ?>
<div class="card">
    <?php if ($has_error): ?>
...
<div class="card">
    <div class="warn">
        ⚠️ <code>mall_products</code> 테이블에 <code>retail_discount_allowed</code>, <code>wholesale_discount_allowed</code> 컬럼(기본값 1)을 추가합니다. 기존 <code>discount_allowed</code> 컬럼은 그대로 남고, 다른 테이블에는 영향을 주지 않습니다.
    </div>
    <form method="post">
