<?php
// Design Ref: main-office-reports — Cash Disbursement read-only viewer + left store menu.
// Reuses office/cash_disbursement/print_cd.php via the mo_store_id parameter so the format
...
try {
    ...
    $state = $store_id > 0 ? get_saved_report_state('cd_saved_state', $store_id, $date) : null;

    $print_url  = '../office/cash_disbursement/print_cd.php?mo_store_id=' . $store_id . '&date=' . $date;
    $export_url = '../office/cash_disbursement/export_cd.php?mo_store_id=' . $store_id . '&date=' . $date;
} catch (Throwable $e) {
    $mo_error = $e->getMessage() . ' (' . basename($e->getFile()) . ':' . $e->getLine() . ')';
    error_log('main_office/cash_disbursement.php error: ' . $mo_error);
}

function mo_cd_qs(int $sid, string $d): string {
...
<div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
  <h2 class="h5 fw-bold mb-0"><i class="fa-solid fa-money-bill-wave me-2 text-warning"></i>Cash Disbursement</h2>
...
<div class="d-flex gap-3 align-items-start">
  ...
  <div class="sr-store-menu bg-white rounded-3 shadow-sm border overflow-hidden">
    ...
    <div class="list-group list-group-flush">
      ...
      <a href="?<?php echo mo_cd_qs($sid, $date); ?>"
  ...
  <div class="flex-fill" style="min-width:0;">
    ...
    <div class="bg-white rounded-3 shadow-sm border p-3 mb-3 d-flex align-items-center gap-2 flex-wrap">
      ...
      <a href="?<?php echo mo_cd_qs($store_id, $prev_date); ?>" class="btn btn-sm btn-outline-secondary"><i class="fa-solid fa-chevron-left"></i></a>
      ...
      <a href="?<?php echo mo_cd_qs($store_id, $next_date); ?>"
    ...
    <div class="bg-white rounded-3 shadow-sm border overflow-hidden">
      <iframe src="<?php echo htmlspecialchars($print_url); ?>" style="width:100%;height:1600px;border:none;display:block;"></iframe>
    </div>
