<?php
// Design Ref: main-office-reports — Cheque Expense Report read-only viewer + left store menu.
// Reuses office/cheque_expense_report/print_cer.php via the mo_store_id parameter so the
// format (A4 print layout) is identical to the original (embedded via iframe).
...
try {
    ...
    $state = $store_id > 0 ? get_saved_report_state('cer_saved_state', $store_id, $date) : null;

    $print_url  = '../office/cheque_expense_report/print_cer.php?mo_store_id=' . $store_id . '&date=' . $date;
    $export_url = '../office/cheque_expense_report/export_cer.php?mo_store_id=' . $store_id . '&date=' . $date;
} catch (Throwable $e) {
    $mo_error = $e->getMessage() . ' (' . basename($e->getFile()) . ':' . $e->getLine() . ')';
    error_log('main_office/cheque_expense_report.php error: ' . $mo_error);
}

function mo_cer_qs(int $sid, string $d): string {
...
<div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
  <h2 class="h5 fw-bold mb-0"><i class="fa-solid fa-file-invoice-dollar me-2 text-danger"></i>Cheque Expense Report</h2>
...
<div class="d-flex gap-3 align-items-start">
  ...
  <div class="sr-store-menu bg-white rounded-3 shadow-sm border overflow-hidden">
    ...
    <div class="list-group list-group-flush">
      ...
      <a href="?<?php echo mo_cer_qs($sid, $date); ?>"
  ...
  <div class="flex-fill" style="min-width:0;">
    ...
    <div class="bg-white rounded-3 shadow-sm border p-3 mb-3 d-flex align-items-center gap-2 flex-wrap">
      ...
      <a href="?<?php echo mo_cer_qs($store_id, $prev_date); ?>" class="btn btn-sm btn-outline-secondary"><i class="fa-solid fa-chevron-left"></i></a>
      ...
      <a href="?<?php echo mo_cer_qs($store_id, $next_date); ?>"
