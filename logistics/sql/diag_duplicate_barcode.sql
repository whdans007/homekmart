...
--    RESTRICT FK: lc_inbound / lc_inventory / lc_order_items / lc_box_breaks
--    → 아래 건수가 하나라도 > 0 이면 물리삭제(DELETE) 불가
SELECT p.id, p.name_en, p.is_active,
       (SELECT COUNT(*) FROM lc_inbound     b  WHERE b.product_id  = p.id) AS inbound_cnt,
       (SELECT COUNT(*) FROM lc_inventory   i  WHERE i.product_id  = p.id) AS inventory_lots,
       ...
       (SELECT COUNT(*) FROM lc_box_breaks  bx WHERE bx.product_id = p.id) AS box_break_cnt,
       (SELECT COUNT(*) FROM lc_product_history h WHERE h.product_id = p.id) AS history_cnt
FROM lc_products p
WHERE p.barcode_unit = @bc
   OR p.barcode_box = @bc
   OR p.barcode_logistics = @bc
