...
/**
 * 물류센터 DB 마이그레이션 v4
 * lc_products: barcode_multi 컬럼 제거
 * 접속: http://서버주소/sunset/logistics/sql/run_migration_v4.php
 * 주의: 실행 후 이 파일을 삭제하세요.
 */
...
$col_check = $conn->query(
    "SELECT COUNT(*) AS cnt FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'lc_products' AND COLUMN_NAME = 'barcode_multi'"
);
...
} elseif ($conn->query("ALTER TABLE lc_products DROP COLUMN barcode_multi")) {
    $status = 'OK';
