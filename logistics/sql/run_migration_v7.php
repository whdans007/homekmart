...
/**
 * 물류센터 DB 마이그레이션 v7
 * lc_products.requires_expiry 컬럼 추가
 * 접속: http://서버주소/sunset/logistics/sql/run_migration_v7.php
 * 주의: 실행 후 이 파일을 삭제하세요.
 */
...
$col = $conn->query("SELECT COUNT(*) AS cnt FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'lc_products' AND COLUMN_NAME = 'requires_expiry'");
$col_exists = (int)$col->fetch_assoc()['cnt'];

if ($col_exists) {
    $steps[] = ['SKIP', 'lc_products.requires_expiry 컬럼이 이미 존재합니다.'];
} elseif ($conn->query("ALTER TABLE lc_products
    ADD COLUMN requires_expiry TINYINT(1) NOT NULL DEFAULT 0
    COMMENT '입고 시 유통기한 필수 입력 여부' AFTER min_stock")) {
    $steps[] = ['OK', 'lc_products.requires_expiry 컬럼 추가 완료'];
} else {
