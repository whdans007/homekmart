 ...
 *   1) lc_inbound / lc_inventory / lc_order_items / lc_box_breaks 의 product_id 를
 *      오타 상품(#merge) → 정상 상품(#keep) 으로 재배정
 *   2) 오타 상품은 is_active=0 처리 + 오타 바코드 컬럼만 해제 (이력 보존을 위해 행은 삭제하지 않음)
 *   3) lc_product_history 는 그대로 두어 오타 상품 자체의 등록 이력을 보존, keep 상품 쪽에 병합 이력 1건 기록
 *   4) 병합 전/후 lc_inventory 수량 합계(quantity_in/out)가 동일한지 검증 후에만 COMMIT
 *
 * 접속: http://main.homekmart.net/logistics/sql/fix_merge_duplicate_product_4801981127191.php
...
if ($keep && $merge) {
    ...
    $before = $conn->query(
        "SELECT COALESCE(SUM(quantity_in),0) tin, COALESCE(SUM(quantity_out),0) tout, COUNT(*) cnt
         FROM lc_inventory WHERE product_id IN ($keep_id, $merge_id)"
    )->fetch_assoc();
    ...
    foreach (['lc_inbound', 'lc_inventory', 'lc_order_items', 'lc_box_breaks'] as $t) {
    ...
    if ($ok) {
        ...
        $sql = "UPDATE lc_products SET is_active = 0, name_en = ?" .
    ...
    if ($ok) {
        ...
        $st = $conn->prepare(
            "INSERT INTO lc_product_history (product_id, action, field_name, field_label, old_value, new_value)
             VALUES (?, 'update', 'merge', '중복상품 병합', ?, ?)"
        );
        $st->bind_param('iss', $keep_id, $oldVal, $newVal);
        $st->execute();
        $st->close();
        step($steps, 'OK', "병합 이력 기록 완료 (lc_product_history)");
    }

    if ($ok) {
        $after = $conn->query(
            "SELECT COALESCE(SUM(quantity_in),0) tin, COALESCE(SUM(quantity_out),0) tout, COUNT(*) cnt
             FROM lc_inventory WHERE product_id = $keep_id"
        )->fetch_assoc();
