...
//    잘못된 바코드 칸만 정정해야 합니다.
//
// product_id 를 저장하는 테이블(전수): lc_inbound / lc_inventory /
//   lc_order_items / lc_box_breaks / lc_product_history
// ============================================================
...
$REF_TABLES = ['lc_inbound', 'lc_inventory', 'lc_order_items', 'lc_box_breaks', 'lc_product_history'];
$BC_COLS = ['barcode_unit', 'barcode_box', 'barcode_logistics'];

function h($s) { return htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8'); }

function load_product(mysqli $conn, int $id): ?array {
    $st = $conn->prepare("SELECT id, name_en, name_ko, is_active, barcode_unit, barcode_box, barcode_logistics FROM lc_products WHERE id = ?");
...
try {
    ...
    if ($keep <= 0 || $remove <= 0) {
        ...
        if ($bc !== '') {
            $st = $conn->prepare("SELECT id, name_en, name_ko, is_active, barcode_unit, barcode_box, barcode_logistics
                                  FROM lc_products WHERE barcode_unit = ? OR barcode_box = ? OR barcode_logistics = ? ORDER BY id");
    ...
    if ($do_merge) {
        ...
        try {
            ...
            if ($sets) {
                $sql = "UPDATE lc_products SET " . implode(', ', $sets) . " WHERE id = ?";
                $vals[] = $keep; $types .= 'i';
                $up = $conn->prepare($sql);
            ...
            $del = $conn->prepare("DELETE FROM lc_products WHERE id = ?");
    ...
    <table class="plan">
      ...
      <tbody>
        <tr><td>입고 (lc_inbound)</td><td><?php echo (int)$Rc['inbound_cnt']; ?> 건 이관</td></tr>
        <tr><td>재고 로트 (lc_inventory)</td><td><?php echo (int)$Rc['inventory_lots']; ?> 건 (현재고 <?php echo number_format((float)$Rc['stock_remain']); ?>) 이관</td></tr>
        <tr><td>주문 상세 (lc_order_items)</td><td><?php echo (int)$Rc['order_items_cnt']; ?> 건 이관</td></tr>
        <tr><td>개봉이력 (lc_box_breaks)</td><td><?php echo (int)$Rc['box_break_cnt']; ?> 건 이관</td></tr>
        <tr><td>변경이력 (lc_product_history)</td><td><?php echo (int)$Rc['history_cnt']; ?> 건 이관</td></tr>
