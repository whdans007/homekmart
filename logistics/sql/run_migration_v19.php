...
/**
 * 물류센터 DB 마이그레이션 v19
 * lc_orders에 deleted_at / deleted_by 컬럼 추가 (소프트 삭제/복구)
 * 접속: http://서버주소/logistics/sql/run_migration_v19.php
 * 주의: 실행 후 이 파일을 삭제하세요.
 */
...
if ((int)$row['cnt'] > 0) {
    $steps[] = ['SKIP', "lc_orders.deleted_at 컬럼이 이미 존재합니다."];
} elseif ($conn->query("
...
")) {
    $steps[] = ['OK', "lc_orders.deleted_at / deleted_by 컬럼 추가 완료"];
} else {
