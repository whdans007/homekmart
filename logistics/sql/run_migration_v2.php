 ...
 * 접속: http://서버주소/sunset/logistics/sql/run_migration_v2.php
 * 주의: 실행 후 이 파일을 삭제하세요.
 */
require_once __DIR__ . '/../../config/db_config.php';

$steps = [
    'lc_brands 생성' => "CREATE TABLE IF NOT EXISTS lc_brands (
    ...
    'lc_categories 생성' => "CREATE TABLE IF NOT EXISTS lc_categories (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    '바코드 4종 컬럼 추가' => "ALTER TABLE lc_products
    ...
    '기존 barcode → barcode_unit 복사' => "UPDATE lc_products SET barcode_unit = barcode WHERE barcode IS NOT NULL AND barcode != ''",

    'barcode 컬럼 제거' => "ALTER TABLE lc_products DROP COLUMN barcode",

    'sku 컬럼 제거' => "ALTER TABLE lc_products DROP COLUMN sku",

    'supplier_id 컬럼 제거' => "ALTER TABLE lc_products DROP COLUMN supplier_id",

    'cost_price 컬럼 제거' => "ALTER TABLE lc_products DROP COLUMN cost_price",

    'selling_price 컬럼 제거' => "ALTER TABLE lc_products DROP COLUMN selling_price",

    'category 텍스트 컬럼 제거' => "ALTER TABLE lc_products DROP COLUMN category",

    'brand FK 추가' => "ALTER TABLE lc_products
        ADD CONSTRAINT fk_lc_products_brand FOREIGN KEY (brand_id) REFERENCES lc_brands(id) ON DELETE SET NULL",

    'category FK 추가' => "ALTER TABLE lc_products
...
try {
    ...
    $db_ver = $row['v'] ?? '';
    $res = $conn_check->query("SHOW COLUMNS FROM lc_products");
    if ($res) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'run') {
    try {
        ...
        // 변경 후 컬럼 목록 갱신
        $res = $conn->query("SHOW COLUMNS FROM lc_products");
        $current_cols = [];
...
<div class="card">
    ...
    <?php else: ?>
    <div class="success-box">✅ 마이그레이션 v2 완료! <a href="<?php echo '/sunset/logistics/login.php'; ?>">물류센터 →</a></div>
...
<!-- 현재 lc_products 컬럼 상태 -->
<div class="card">
    <p style="font-size:.875rem;font-weight:600;color:#374151;margin:0 0 .75rem">lc_products 현재 컬럼</p>
...
<div class="card">
    <div class="success-box">✅ 이미 v2 마이그레이션이 완료된 상태입니다. <a href="/sunset/logistics/login.php">물류센터 →</a></div>
</div>
...
<div class="card">
    ...
    <form method="post">
        ...
        <button type="submit" onclick="return confirm('lc_products v2 마이그레이션을 실행하시겠습니까?\n\n주의: cost_price, selling_price, sku, supplier_id 컬럼이 삭제됩니다.')">
            마이그레이션 v2 실행
        </button>
    </form>
</div>
