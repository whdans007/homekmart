...
$page_title = t('logistics.category.page_title');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if ($name_en !== '') {
            ...
            lc_set_flash('success', t('logistics.category.add_success', ['name' => $name_en]));
        }
    } elseif ($action === 'edit') {
        ...
        $name_ko = trim($_POST['name_ko'] ?? '') ?: null;
        if ($cid > 0 && $name_en !== '') {
            $st = $conn->prepare("UPDATE lc_categories SET name_en = ?, name_ko = ? WHERE id = ?");
            $st->bind_param('ssi', $name_en, $name_ko, $cid); $st->execute(); $st->close();
            lc_set_flash('success', t('logistics.category.edit_success', ['name' => $name_en]));
        }
    } elseif ($action === 'delete') {
        $cid = (int)($_POST['cat_id'] ?? 0);
        $st = $conn->prepare("DELETE FROM lc_categories WHERE id = ?");
        $st->bind_param('i', $cid); $st->execute(); $st->close();
        lc_set_flash('success', t('logistics.category.delete_success'));
    }
    $conn->close();
...
try {
    ...
    $cnt = $conn->prepare("SELECT COUNT(*) FROM lc_categories c $where");
    ...
    $sql = "SELECT c.id, c.name_en, c.name_ko, COUNT(p.id) AS product_count
            FROM lc_categories c LEFT JOIN lc_products p ON p.category_id = c.id
            $where
...
<div class="flex items-center gap-3 shrink-0">
    ...
    <h2 class="text-xl font-bold text-gray-900"><?php echo htmlspecialchars(t('logistics.category.title')); ?></h2>
</div>
...
<div class="bg-white rounded-lg border border-gray-200 px-3 py-2 shrink-0">
    <form method="post" id="catForm" onsubmit="return prepareCatSubmit()">
        ...
        <div class="flex gap-2 items-center flex-wrap">
            <span class="text-xs font-semibold text-gray-500 mr-1"><i class="fas fa-plus-circle text-teal-500 mr-1"></i><?php echo htmlspecialchars(t('logistics.category.register')); ?></span>
            <input type="text" name="name_en" id="catEnInput" placeholder="<?php echo htmlspecialchars(t('logistics.category.english_name')); ?>"
            ...
            <input type="text" name="name_ko" id="catKoInput" placeholder="<?php echo htmlspecialchars(t('logistics.category.korean_name_optional')); ?>"
            ...
            <button type="button" onclick="romanizeCatName()" title="<?php echo htmlspecialchars(t('logistics.category.romanize')); ?>"
                    class="px-3 py-1.5 bg-blue-500 text-white text-xs font-semibold rounded-md hover:bg-blue-600 whitespace-nowrap transition-colors"><?php echo htmlspecialchars(t('logistics.category.romanize')); ?></button>
            <button type="submit" class="px-4 py-1.5 bg-teal-600 text-white text-sm rounded-md hover:bg-teal-700 whitespace-nowrap"><?php echo htmlspecialchars(t('logistics.category.register_button')); ?></button>
        </div>
    </form>
</div>
...
<form method="get" class="bg-white rounded-lg border border-gray-200 px-3 py-2 shrink-0">
    <div class="flex flex-wrap items-center gap-2">
        <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>"
               placeholder="<?php echo htmlspecialchars(t('logistics.category.search_placeholder')); ?>"
               class="border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 w-56">
        <button type="submit" class="px-3 py-1.5 bg-teal-600 text-white text-sm rounded-md hover:bg-teal-700"><i class="fas fa-search mr-1"></i><?php echo htmlspecialchars(t('logistics.category.search')); ?></button>
        <a href="<?php echo LC_BASE; ?>/category_manage.php" class="px-3 py-1.5 bg-gray-100 text-gray-600 text-sm rounded-md hover:bg-gray-200"><?php echo htmlspecialchars(t('logistics.category.reset')); ?></a>
    </div>
</form>
...
<div class="bg-white rounded-lg border border-gray-200 overflow-hidden flex flex-col flex-1 min-h-0">
    <div class="px-4 py-3 border-b border-gray-100 text-sm text-gray-500 shrink-0"><?php echo htmlspecialchars(t('logistics.category.total', ['count' => number_format($total)])); ?></div>
    <div class="overflow-auto flex-1 min-h-0">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 sticky top-0 z-10"><tr>
                <th class="px-4 py-3 text-left text-xs text-gray-500 font-medium"><?php echo htmlspecialchars(t('logistics.category.english_name')); ?></th>
                <th class="px-4 py-3 text-left text-xs text-gray-500 font-medium"><?php echo htmlspecialchars(t('logistics.category.korean_name')); ?></th>
                <th class="px-4 py-3 text-center text-xs text-gray-500 font-medium"><?php echo htmlspecialchars(t('logistics.category.products')); ?></th>
                <th class="px-4 py-3 text-center text-xs text-gray-500 font-medium"><?php echo htmlspecialchars(t('logistics.category.manage')); ?></th>
            </tr></thead>
            ...
            <tr><td colspan="4" class="px-4 py-10 text-center text-gray-400">
                ...
                <span class="font-medium text-gray-600">"<?php echo htmlspecialchars($search); ?>"</span> <?php echo htmlspecialchars(t('logistics.category.no_search_results')); ?>
                <?php else: ?>
                <i class="fas fa-sitemap text-3xl mb-2 block text-gray-300"></i>
                <?php echo htmlspecialchars(t('logistics.category.empty')); ?>
                <?php endif; ?>
            </td></tr>
            <?php endif; ?>
                ...
                <td class="px-4 py-3 text-center">
                    <div class="flex items-center justify-center gap-3">
                        <button type="button"
                                ...
                                class="text-teal-600 hover:text-teal-800 text-xs" title="<?php echo htmlspecialchars(t('logistics.category.edit')); ?>"><i class="fas fa-edit"></i></button>
                        ...
                        <form method="post" class="inline" onclick="event.stopPropagation()">
                            ...
                            <button type="submit" onclick="return confirm(<?php echo htmlspecialchars(json_encode(t('logistics.category.delete_confirm')), ENT_QUOTES); ?>)" class="text-red-400 hover:text-red-600 text-xs" title="<?php echo htmlspecialchars(t('logistics.category.delete')); ?>"><i class="fas fa-trash"></i></button>
                        </form>
                        <?php else: ?>
                        <span class="text-xs px-2 py-1 rounded-full bg-gray-100 text-gray-500" title="<?php echo htmlspecialchars(t('logistics.category.in_use_title')); ?>"><?php echo htmlspecialchars(t('logistics.category.in_use', ['count' => (int)$c['product_count']])); ?></span>
                        <?php endif; ?>
                    </div>
                </td>
            </tr>
    ...
    <div class="px-4 py-3 border-t border-gray-100 flex items-center justify-center gap-1 shrink-0">
        ...
        <a href="?page=<?php echo $block_start - $window; ?>&<?php echo http_build_query($qs); ?>"
            class="w-8 h-8 flex items-center justify-center rounded border border-gray-300 bg-white text-gray-600 hover:bg-gray-50 text-sm transition-colors" title="<?php echo htmlspecialchars(t('logistics.category.previous_pages')); ?>">
        ...
        <a href="?page=<?php echo $block_end + 1; ?>&<?php echo http_build_query($qs); ?>"
            class="w-8 h-8 flex items-center justify-center rounded border border-gray-300 bg-white text-gray-600 hover:bg-gray-50 text-sm transition-colors" title="<?php echo htmlspecialchars(t('logistics.category.next_pages')); ?>">
...
<div id="editModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-40 px-4">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h3 class="text-sm font-semibold text-gray-800"><?php echo htmlspecialchars(t('logistics.category.edit_title')); ?></h3>
        ...
        <form method="post" class="p-5 space-y-4" onsubmit="return prepareEditSubmit()">
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1"><?php echo htmlspecialchars(t('logistics.category.name_english')); ?> <span class="text-red-400">*</span></label>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1"><?php echo htmlspecialchars(t('logistics.category.name_korean')); ?></label>
                <div class="flex gap-2">
                    ...
                    <button type="button" onclick="romanizeEditName()" title="<?php echo htmlspecialchars(t('logistics.category.romanize')); ?>"
                            class="px-3 py-2 bg-blue-500 text-white text-xs font-semibold rounded-md hover:bg-blue-600 whitespace-nowrap"><?php echo htmlspecialchars(t('logistics.category.romanize')); ?></button>
                </div>
            </div>
            <div class="flex justify-end gap-2 pt-2">
                <button type="button" onclick="closeEditModal()" class="px-4 py-2 text-sm text-gray-600 border border-gray-300 rounded-md hover:bg-gray-50"><?php echo htmlspecialchars(t('logistics.category.cancel')); ?></button>
                <button type="submit" class="px-4 py-2 bg-teal-600 text-white text-sm rounded-md hover:bg-teal-700"><?php echo htmlspecialchars(t('logistics.category.save')); ?></button>
            </div>
        </form>
    </div>
</div>
...
<div id="productsModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-40 px-4">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-lg flex flex-col" style="max-height:80vh;">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
            <h3 class="text-sm font-semibold text-gray-800"><?php echo htmlspecialchars(t('logistics.category.products_using')); ?> <span id="productsCatName" class="text-teal-600"></span></h3>
        ...
        <div id="productsModalBody" class="p-5 overflow-auto flex-1 min-h-0">
            <div class="text-center text-gray-400 py-8"><i class="fas fa-spinner fa-spin mr-2"></i><?php echo htmlspecialchars(t('logistics.category.loading')); ?></div>
        </div>
        <div class="px-5 py-3 border-t border-gray-100 flex justify-end shrink-0">
            <button type="button" onclick="closeProductsModal()" class="px-4 py-2 text-sm text-gray-600 border border-gray-300 rounded-md hover:bg-gray-50"><?php echo htmlspecialchars(t('logistics.category.close')); ?></button>
        </div>
    </div>
</div>
...
(function() {
    ...
    window.openProductsModal = function(catId, catName, count) {
        document.getElementById('productsCatName').textContent = <?php echo json_encode(t('logistics.category.products_count')); ?>.replace('{name}', catName).replace('{count}', count);
        var body = document.getElementById('productsModalBody');
        body.innerHTML = '<div class="text-center text-gray-400 py-8"><i class="fas fa-spinner fa-spin mr-2"></i><?php echo htmlspecialchars(t('logistics.category.loading')); ?></div>';
        ...
        fetch(LC_BASE + '/ajax_inuse_products.php?type=category&id=' + encodeURIComponent(catId))
            ...
            .then(function(data) {
                if (!data.ok) { body.innerHTML = '<div class="text-center text-red-500 py-8"><?php echo htmlspecialchars(t('logistics.category.load_failed')); ?></div>'; return; }
                if (!data.products.length) { body.innerHTML = '<div class="text-center text-gray-400 py-8"><?php echo htmlspecialchars(t('logistics.category.no_products')); ?></div>'; return; }
                var html = '<table class="w-full text-sm"><thead class="bg-gray-50"><tr>'
                    + '<th class="px-3 py-2 text-left text-xs text-gray-500 font-medium"><?php echo htmlspecialchars(t('logistics.category.product')); ?></th>'
                    + '<th class="px-3 py-2 text-left text-xs text-gray-500 font-medium"><?php echo htmlspecialchars(t('logistics.category.barcode')); ?></th>'
                    + '<th class="px-3 py-2 text-center text-xs text-gray-500 font-medium"><?php echo htmlspecialchars(t('logistics.category.status')); ?></th>'
                ...
                data.products.forEach(function(p) {
                    ...
                    var badge = (parseInt(p.is_active, 10) === 1)
                        ? '<span class="text-xs px-2 py-0.5 rounded-full bg-green-100 text-green-700"><?php echo htmlspecialchars(t('logistics.category.active')); ?></span>'
                        : '<span class="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-500"><?php echo htmlspecialchars(t('logistics.category.inactive')); ?></span>';
            ...
            .catch(function() { body.innerHTML = '<div class="text-center text-red-500 py-8"><?php echo htmlspecialchars(t('logistics.category.load_failed')); ?></div>'; });
    };
