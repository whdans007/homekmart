...
try {
    ...
    $expired_count = (int)$conn->query(
        "SELECT COUNT(*) FROM lc_inventory i
         JOIN lc_inbound ib ON i.inbound_id = ib.id
    ...
    $pending_count = (int)$conn->query(
        "SELECT COUNT(*) FROM lc_orders WHERE status = 'pending'"
    )->fetch_row()[0];

    // 출고 대기 주문 수 (승인됨)
    $approved_count = (int)$conn->query(
        "SELECT COUNT(*) FROM lc_orders WHERE status = 'approved'"
    )->fetch_row()[0];

    // 전체 상품 수 / 재고 있는 상품 수
    $total_products = (int)$conn->query("SELECT COUNT(*) FROM lc_products WHERE is_active = 1")->fetch_row()[0];
    $stocked_products = (int)$conn->query(
        "SELECT COUNT(DISTINCT i.product_id) FROM lc_inventory i
         JOIN lc_inbound ib ON i.inbound_id = ib.id
    ...
    if (lc_is_store_user()) {
        ...
        $st = $conn->prepare(
            "SELECT o.id, o.order_date, o.status, o.total_amount,
             ...
             FROM lc_orders o
             LEFT JOIN lc_order_items i ON o.id = i.order_id
             WHERE o.store_id = ?
