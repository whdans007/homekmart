 ...
 *
 * 사용법: /logistics/diag_stock.php?barcode=4800014147090
...
try {
    ...
    $sections['⑤ 입고 원본 (lc_inbound) — 58 BOX가 2건이면 중복/오등록 의심, 날짜·공급처·원가 비교'] = q($conn,
        "SELECT ib.id AS inbound_id, ib.inbound_date, ib.lot_number, ib.expiry_date,
         ...
         FROM lc_inbound ib
         JOIN lc_products p    ON ib.product_id = p.id
         LEFT JOIN lc_inventory inv ON inv.inbound_id = ib.id
         WHERE p.barcode_unit = ? OR p.barcode_box = ? OR p.barcode_logistics = ?
