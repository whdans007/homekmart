...
function lc_safe_return_url(string $val): string {
    if ($val === '') return '';
...
$return_url = lc_safe_return_url((string)($_POST['return'] ?? $_GET['return'] ?? ''));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'confirm') {
    ...
    try {
        ...
        $st = $conn->prepare(
            "UPDATE lc_inbound_batches SET is_confirmed=1, confirmed_at=?, confirmed_by=? WHERE id=? AND is_confirmed=0"
        );
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_batch') {
    ...
    try {
        ...
        // 출고된 재고가 있으면 삭제 불가
        $used = $conn->query(
            "SELECT COUNT(*) FROM lc_inventory inv
             JOIN lc_inbound i ON inv.inbound_id = i.id
        ...
        // lc_inventory 삭제 (inbound_id 기준)
        $conn->query(
            "DELETE inv FROM lc_inventory inv
             JOIN lc_inbound i ON inv.inbound_id = i.id
             WHERE i.batch_id = $batch_id"
        );
        // lc_inbound 아이템 삭제
        $conn->query("DELETE FROM lc_inbound WHERE batch_id = $batch_id");
        // 배치 헤더 삭제
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'unconfirm') {
    ...
    try {
        $conn = get_lc_db();
        $st = $conn->prepare(
            "UPDATE lc_inbound_batches SET is_confirmed=0, confirmed_at=NULL, confirmed_by=NULL WHERE id=? AND is_confirmed=1"
        );
    ...
    <div class="overflow-x-auto">
        <table id="itemsTable" class="w-full text-sm" style="min-width:1100px">
            ...
            <?php
                ...
                $unit     = lc_valid_unit($row['inbound_unit'] ?? 'PCS');
                $is_bundle = lc_is_bundle_unit($unit); // BOX/PACK 공통 처리 (pack-unit §4)
                ...
                $expClass  = lc_expiry_class($row['expiry_date']);
