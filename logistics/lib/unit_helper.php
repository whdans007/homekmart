...
//
// 유효 단가 규칙 (Design §3.2):
//   lot unit=BOX                          → lc_inbound.cost_price (BOX 단가)
//   lot unit=PCS, inbound_unit=PCS        → lc_inbound.cost_price
//   lot unit=PCS, inbound_unit=BOX (개봉 파생 lot) → lc_inbound.cost_price_pcs
...
// FEFO/평균원가 쿼리 공용 유효단가 CASE식 (i=lc_inventory, b=lc_inbound 별칭 가정)
 ...
 */
function lc_is_bundle_unit(?string $unit): bool {
 ...
 */
function lc_valid_unit(?string $unit, string $default = LC_UNIT_PCS): string {
 ...
 */
function lc_pcs_cost(float $box_cost, int $ppb): float {
 ...
 * 두 경우 모두 최소 1로 보정한다.
 */
function lc_resolve_row_ppb($posted, $fallback): int {
 ...
 * (재고 잔량 > 0 lot 기준)
 */
function lc_get_avg_cost_by_unit(mysqli $conn, int $product_id): array {
 ...
 * lc_fifo_ship_allow_negative()의 단위 버전: WHERE i.unit = $unit 인 lot에서만 차감하고
 * 유효단가(LC_EFFECTIVE_COST_SQL)를 cost_price로 반환한다. 트랜잭션 내에서만 호출할 것.
 * Plan SC-5: BOX 주문 → BOX lot만, PCS 주문 → PCS lot만 차감
 */
function lc_unit_fifo_ship_allow_negative(mysqli $conn, int $product_id, int $qty_needed, string $unit): array {
    $unit = lc_valid_unit($unit);
    ...
    if ($remaining > 0) {
        ...
        if ($lot) {
            ...
            $today = date('Y-m-d');
            $ins = $conn->prepare(
                "INSERT INTO lc_inbound (inbound_date, product_id, lot_number, quantity, inbound_unit, pieces_per_box, cost_price, cost_price_pcs)
                 VALUES (?, ?, 'ADJUST', 0, ?, 1, ?, ?)"
            );
            ...
            $insInv = $conn->prepare(
                "INSERT INTO lc_inventory (inbound_id, product_id, unit, lot_number, quantity_in, quantity_out)
                 VALUES (?, ?, ?, 'ADJUST', 0, ?)"
            );
 ...
 */
function lc_unit_fefo_preview_allow_negative(mysqli $conn, int $product_id, int $qty_needed, string $unit): array {
    $unit = lc_valid_unit($unit);
 ...
 */
function lc_execute_box_break(mysqli $conn, int $inventory_id, int $boxes, int $damaged, string $notes = '', ?int $user_id = null): array {
    ...
    if (!$lot || !lc_is_bundle_unit($lot['unit'])) {
    ...
    // 2) BOX lot 차감
    $st = $conn->prepare("UPDATE lc_inventory SET quantity_out = quantity_out + ? WHERE id = ?");
    ...
    if ($pcs_created > 0) {
        $st = $conn->prepare(
            "INSERT INTO lc_inventory (inbound_id, product_id, unit, lot_number, expiry_date, storage_location, quantity_in, quantity_out)
             VALUES (?, ?, 'PCS', ?, ?, ?, ?, 0)"
        );
