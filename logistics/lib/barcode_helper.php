 ...
 *
 * 상품 마스터(lc_products) 입력 루틴이 여러 곳(product_add.php, ajax/add_product.php,
 ...
 *                 빈 배열이면 중복 없음.
 */
function lc_find_barcode_conflicts(mysqli $conn, array $barcodes, int $exclude_product_id = 0): array
{
    // 빈값 제거 + 중복 값 정규화
    $vals = [];
 ...
 *
 * @param array $conflicts lc_find_barcode_conflicts() 반환값
 * @return string
 */
function lc_format_barcode_conflict_msg(array $conflicts): string
{
    $parts = [];
