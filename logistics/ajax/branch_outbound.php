...
if ($action === 'get_product_stock') {
    ...
    try {
        ...
        if ($product) {
            ...
            $cost  = lc_get_avg_cost_by_unit($conn, $product_id);
...
if ($action === 'get_fefo_preview') {
    ...
    try {
        ...
        $preview = lc_unit_fefo_preview_allow_negative($conn, $product_id, $qty, $unit);
        $conn->close();
        echo json_encode([
            'success' => true,
            'picks' => $preview['picks'],
...
if ($action === 'submit' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Plan FR-06: lc_orders 1건(status='shipped') 생성, total_amount는 처리 후 갱신
        $ins_order = $conn->prepare(
            "INSERT INTO lc_orders (order_date, store_id, status, total_amount, notes, created_by, shipped_at)
        ...
        $ppb_map = lc_bo_ppb_map($conn, array_column($cart, 'product_id'));

        $total_amount = 0.0;
        foreach ($cart as $line) {
            // Design Ref: box-pcs-unit §4.3 / Plan SC-5 — 동일 단위 lot에서만 FEFO 차감 (음수 허용)
            $deductions = lc_unit_fifo_ship_allow_negative($conn, $line['product_id'], $line['quantity'], $line['unit']);
        ...
        $upd_order = $conn->prepare("UPDATE lc_orders SET total_amount = ? WHERE id = ?");
...
function lc_bo_avg_cost(mysqli $conn, int $product_id, string $unit = LC_UNIT_PCS): float {
    $cost = lc_get_avg_cost_by_unit($conn, $product_id);
    return (float)($cost[lc_valid_unit($unit)] ?? 0);
}

// 상품별 ppb 스냅샷 맵 (lc_order_items.pieces_per_box 기록용)
function lc_bo_ppb_map(mysqli $conn, array $product_ids): array {
    $product_ids = array_values(array_unique(array_map('intval', $product_ids)));
    if (empty($product_ids)) return [];
    $in = implode(',', $product_ids);
    $map = [];
    $res = $conn->query("SELECT id, GREATEST(1, IFNULL(pieces_per_box, 1)) AS ppb FROM lc_products WHERE id IN ($in)");
    foreach ($res->fetch_all(MYSQLI_ASSOC) as $r) $map[(int)$r['id']] = (int)$r['ppb'];
    return $map;
}

// items JSON 파싱 + 검증 → [['product_id'=>..,'quantity'=>..,'unit'=>..], ...]
function lc_bo_parse_items(string $json): array {
    ...
    foreach ($items as $it) {
        $pid = (int)($it['product_id'] ?? 0);
        $qty = (int)($it['quantity'] ?? 0);
        $unit = lc_valid_unit($it['unit'] ?? '', LC_UNIT_PCS);
        if ($pid > 0 && $qty > 0) $cart[] = ['product_id' => $pid, 'quantity' => $qty, 'unit' => $unit];
    }
    return $cart;
}
...
function lc_bo_insert_items(mysqli $conn, int $order_id, array $cart): float {
    $ins = $conn->prepare(
        "INSERT INTO lc_order_items (order_id, product_id, quantity, order_unit, pieces_per_box, unit_price)
         VALUES (?, ?, ?, ?, ?, ?)"
    );
    ...
    $total = 0.0;
    foreach ($cart as $line) {
        $price = lc_bo_avg_cost($conn, $line['product_id'], $line['unit']);
...
// Plan SC-1: 저장 시 재고 차감 없음 — lc_orders(status='draft') + items만 기록
if ($action === 'save_draft' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $total = lc_bo_insert_items($conn, $draft_id, $cart);

        $upd = $conn->prepare("UPDATE lc_orders SET total_amount = ? WHERE id = ?");
...
if ($action === 'get_draft') {
    ...
    try {
        ...
        $st = $conn->prepare("SELECT id, store_id, notes, status FROM lc_orders WHERE id = ?");
...
if ($action === 'update_draft' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $st = $conn->prepare("SELECT status FROM lc_orders WHERE id = ? FOR UPDATE");
        $st->bind_param('i', $draft_id);
        $st->execute();
        $order = $st->get_result()->fetch_assoc();
        $st->close();
        ...
        $del = $conn->prepare("DELETE FROM lc_order_items WHERE order_id = ?");
        ...
        $upd = $conn->prepare("UPDATE lc_orders SET store_id = ?, notes = ?, total_amount = ? WHERE id = ?");
...
if ($action === 'delete_draft' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $del2 = $conn->prepare("DELETE FROM lc_orders WHERE id = ?");
...
if ($action === 'ship_draft' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $st = $conn->prepare("SELECT id, product_id, quantity, COALESCE(order_unit, 'PCS') AS order_unit FROM lc_order_items WHERE order_id = ?");
        $st->bind_param('i', $draft_id);
        $st->execute();
        $items = $st->get_result()->fetch_all(MYSQLI_ASSOC);
        $st->close();
        ...
        $upd_item = $conn->prepare("UPDATE lc_order_items SET unit_price = ? WHERE id = ?");
        $ins_lot  = $conn->prepare(
            "INSERT INTO lc_order_item_lots (order_item_id, inventory_id, inbound_id, quantity, cost_price)
             VALUES (?, ?, ?, ?, ?)"
        );
        ...
        foreach ($items as $line) {
            ...
            $unit = lc_valid_unit($line['order_unit'] ?? '', LC_UNIT_PCS);

            // Design §3.4 / Plan SC-5 — 출고 시점 재고 기준, 동일 단위 lot에서만 FEFO 차감 (음수 허용)
            $deductions = lc_unit_fifo_ship_allow_negative($conn, $pid, $qty, $unit);
