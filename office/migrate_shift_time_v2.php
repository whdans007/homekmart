...
try {
    ...
    $required = ["'8AM~5PM'", "'8AM~8PM'", "'8PM~8AM'"];
    $missing  = array_filter($required, fn($v) => !str_contains($col['Type'], trim($v, "'")));

    if (empty($missing)) {
        echo "\n✓ 이미 모든 값이 ENUM에 포함되어 있습니다. 마이그레이션 불필요.\n";
    } else {
        echo "누락된 값: " . implode(', ', $missing) . "\n";
        echo "\n[2단계] ENUM 확장 실행...\n";
